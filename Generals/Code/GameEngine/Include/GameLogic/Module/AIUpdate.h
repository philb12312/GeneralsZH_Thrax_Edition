...
class PathNode;
struct AsyncPathfindJob;
struct AsyncPathfindResult;
...
public:
	...
	Bool buildAsyncPathfindJob( AsyncPathfindJob &job );
	Bool hasAsyncPathfindInFlight() const { return m_asyncPathRequestSerial != 0 && m_asyncPathRequestSerial == m_pathRequestSerial; }
	Bool isAsyncPathfindResultCurrent( const AsyncPathfindResult &result ) const;
	Bool applyAsyncPathfindResult( const AsyncPathfindResult &result );
	...
	void setDebugBotOrderContext(const char *reason, const char *verb, const Coord3D *targetPos, Int relatedPlayerIndex = -1);
	void clearDebugBotOrderContext();
	void formatDebugBotOrderContext(AsciiString &summary) const;
...
private:
	...
	CommandSourceType		m_lastCommandSource;			/**< Keep track of the source of the last command we got.
																								...
																								*/
	const char					*m_debugBotOrderReason;	///< Last high-level bot order reason tag (string literal).
	const char					*m_debugBotOrderVerb;		///< Last high-level bot order verb tag (string literal).
	UnsignedInt				m_debugBotOrderSequence;	///< Sequence number for high-level bot orders on this object.
	UnsignedInt				m_debugBotOrderFrame;	///< Frame when the last high-level bot order was issued.
	Coord3D						m_debugBotOrderIssuePos;	///< Object position when the last high-level bot order was issued.
	Coord3D						m_debugBotOrderTargetPos;	///< Target position of the last high-level bot order.
	Int							m_debugBotOrderRelatedPlayerIndex;	///< Related player for the order (enemy/ally), or -1.
	...
	UnsignedInt	m_pathRequestSerial;					///< increments when queued path state changes, used to discard stale async results
	UnsignedInt	m_asyncPathRequestSerial;	///< non-zero while a worker job exists for this request serial

...
//			if(TheGlobalData->m_loadScreenRender != TRUE)
    ...
    if ( (TheGameLogic->getFrame() % 30 == 1) || ( ! ( !TheGameLogic->isGamePaused() && TheGlobalData->m_TiVOFastMode) ) )
    ...
    #endif
		{
			...
			if ((TheGlobalData->m_breakTheMovie == FALSE) && (TheGlobalData->m_disableRender == false) && WW3D::Begin_Render( true, true, Vector3( 0.0f, 0.0f, 0.0f ), TheWaterTransparency->getEffectiveWaterOpacity() ) == WW3D_ERROR_OK)
			{

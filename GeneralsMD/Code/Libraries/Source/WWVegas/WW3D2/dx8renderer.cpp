...
namespace
{
	int g_meshPerfRigidTasks = 0;
	int g_meshPerfSkinMeshes = 0;
	int g_meshPerfSkinVertices = 0;
	int g_meshPerfDecalMeshes = 0;
	int g_meshPerfDelayedPasses = 0;
	double g_meshPerfMsPerTick = 0.0;
	double g_meshPerfRigidStateMs = 0.0;
	double g_meshPerfRigidLightMs = 0.0;
	double g_meshPerfRigidTransformMs = 0.0;
	double g_meshPerfRigidDrawMs = 0.0;
	int g_meshPerfRigidTasksVisited = 0;
	int g_meshPerfRigidOverflowTasks = 0;
	int g_meshPerfRigidDrawCalls = 0;
	int g_meshPerfRigidSortedDrawCalls = 0;
	int g_meshPerfRigidAlphaTasks = 0;

	inline void RecordMeshPerfSub(const char *tag, const LARGE_INTEGER &begin, const LARGE_INTEGER &end, double msPerTick)
	{
		if (msPerTick > 0.0)
		{
			PerfRecordLogicSub(tag, (double)(end.QuadPart - begin.QuadPart) * msPerTick);
		}
	}

	inline void RecordMeshPerfValue(const char *tag, double ms)
	{
		if (ms > 0.0)
		{
			PerfRecordLogicSub(tag, ms);
		}
	}

	inline void MeshPerfBegin(LARGE_INTEGER &begin)
	{
		if (g_meshPerfMsPerTick > 0.0)
		{
			QueryPerformanceCounter(&begin);
		}
	}

	inline void MeshPerfAccum(double &accum, const LARGE_INTEGER &begin)
	{
		if (g_meshPerfMsPerTick > 0.0)
		{
			LARGE_INTEGER end;
			QueryPerformanceCounter(&end);
			accum += (double)(end.QuadPart - begin.QuadPart) * g_meshPerfMsPerTick;
		}
	}

	inline void ResetMeshRigidPerfBreakdown()
	{
		g_meshPerfRigidStateMs = 0.0;
		g_meshPerfRigidLightMs = 0.0;
		g_meshPerfRigidTransformMs = 0.0;
		g_meshPerfRigidDrawMs = 0.0;
		g_meshPerfRigidTasksVisited = 0;
		g_meshPerfRigidOverflowTasks = 0;
		g_meshPerfRigidDrawCalls = 0;
		g_meshPerfRigidSortedDrawCalls = 0;
		g_meshPerfRigidAlphaTasks = 0;
	}
}
...
void DX8SkinFVFCategoryContainer::Add_Visible_Skin(MeshClass * mesh)
{
	...
	++g_meshPerfSkinMeshes;
	g_meshPerfSkinVertices += mesh->Peek_Model()->Get_Vertex_Count();
}
...
{
	LARGE_INTEGER meshPerfA;
	MeshPerfBegin(meshPerfA);
	...
	MeshPerfAccum(g_meshPerfRigidStateMs, meshPerfA);
	...
	while (prt) {
		...
		*/
		...
		++g_meshPerfRigidTasksVisited;

		if (mesh->Get_Base_Vertex_Offset() == VERTEX_BUFFER_OVERFLOW)	//check if this mesh is valid
		{	//skip this mesh so it gets rendered later after vertices are filled in.
			++g_meshPerfRigidOverflowTasks;
			last_prt = prt;
		...
		LARGE_INTEGER meshTaskA;
		MeshPerfBegin(meshTaskA);
		...
		*/
		...
		MeshPerfAccum(g_meshPerfRigidLightMs, meshPerfA);
		...
		*/
		...
		MeshPerfAccum(g_meshPerfRigidTransformMs, meshPerfA);


	//--------------------------------------------------------------------
		const int meshDrawToken = PerfBeginScopedLogicSub("Cl_Mesh_Rigid_Draw");
...
//--------------------------------------------------------------------
		...
		*/
		...
		if ((!!mesh->Peek_Model()->Get_Flag(MeshGeometryClass::SORT)) && WW3D::Is_Sorting_Enabled()) {
			++g_meshPerfRigidSortedDrawCalls;
...
//--------------------------------------------------------------------
		...
		PerfEndScopedLogicSub(meshDrawToken);
		if (g_meshPerfMsPerTick > 0.0)
		{
			LARGE_INTEGER meshTaskB;
			QueryPerformanceCounter(&meshTaskB);
			PerfRecordNamedCost("Mesh", mesh->Get_Name() ? mesh->Get_Name() : "?", (double)(meshTaskB.QuadPart - meshTaskA.QuadPart) * g_meshPerfMsPerTick);
		}
...
void DX8MeshRendererClass::Flush()
{
	...
	const int rigidTasks = g_meshPerfRigidTasks;
	const int skinMeshes = g_meshPerfSkinMeshes;
	const int skinVertices = g_meshPerfSkinVertices;
	const int decalMeshes = g_meshPerfDecalMeshes;
	const int delayedPasses = g_meshPerfDelayedPasses;
	const double meshMsPerTick = PerfGetFreqQuadPartAsMsMultiplier();
	g_meshPerfMsPerTick = 0.0;
	LARGE_INTEGER meshA, meshB;
	...
	*/
	ResetMeshRigidPerfBreakdown();
	QueryPerformanceCounter(&meshA);
	...
	QueryPerformanceCounter(&meshB);
	RecordMeshPerfSub("Cl_Mesh_Rigid", meshA, meshB, meshMsPerTick);
	RecordMeshPerfValue("Cl_Mesh_Rigid_State", g_meshPerfRigidStateMs);
	RecordMeshPerfValue("Cl_Mesh_Rigid_Light", g_meshPerfRigidLightMs);
	RecordMeshPerfValue("Cl_Mesh_Rigid_Xform", g_meshPerfRigidTransformMs);
	RecordMeshPerfValue("Cl_Mesh_Rigid_Draw", g_meshPerfRigidDrawMs);

	QueryPerformanceCounter(&meshA);
	Render_FVF_Category_Container_List(*texture_category_container_list_skin);
	QueryPerformanceCounter(&meshB);
	RecordMeshPerfSub("Cl_Mesh_Skin", meshA, meshB, meshMsPerTick);

	QueryPerformanceCounter(&meshA);
	Render_Decal_Meshes();
	QueryPerformanceCounter(&meshB);
	RecordMeshPerfSub("Cl_Mesh_Decal", meshA, meshB, meshMsPerTick);
	...
	*/
	...
	QueryPerformanceCounter(&meshB);
	RecordMeshPerfSub("Cl_Mesh_Delay", meshA, meshB, meshMsPerTick);

	QueryPerformanceCounter(&meshA);
	DX8Wrapper::Set_Vertex_Buffer(nullptr);
	DX8Wrapper::Set_Index_Buffer(nullptr,0);
	QueryPerformanceCounter(&meshB);
	RecordMeshPerfSub("Cl_Mesh_Unbind", meshA, meshB, meshMsPerTick);

	PerfRecordRenderFeatureStats("MeshRigid", 0, rigidTasks, texture_category_container_lists_rigid.Count(), 0, 0, 0);
	PerfRecordRenderFeatureStats("MeshRigidBrk", 0, g_meshPerfRigidTasksVisited, g_meshPerfRigidDrawCalls,
		g_meshPerfRigidOverflowTasks, g_meshPerfRigidAlphaTasks, g_meshPerfRigidSortedDrawCalls);
	PerfRecordRenderFeatureStats("MeshSkin", 0, skinMeshes, skinMeshes, 0, 0, skinVertices);
	PerfRecordRenderFeatureStats("MeshDecal", 0, decalMeshes, decalMeshes, 0, 0, 0);
	PerfRecordRenderFeatureStats("MeshDelay", 0, delayedPasses, delayedPasses, 0, 0, 0);

	g_meshPerfRigidTasks = 0;
	g_meshPerfSkinMeshes = 0;
	g_meshPerfSkinVertices = 0;
	g_meshPerfDecalMeshes = 0;
	g_meshPerfDelayedPasses = 0;
	g_meshPerfMsPerTick = 0.0;
}

...
namespace
{
	...
	#define DX8_PERF_JOIN2(a, b) a##b
	#define DX8_PERF_JOIN(a, b) DX8_PERF_JOIN2(a, b)
	#define DX8_PERF_SCOPE(tag) DX8PerfScope DX8_PERF_JOIN(_dx8PerfScope_, __LINE__)(tag)
}
...
IDirect3DVertexBuffer8*	DX8Wrapper::CurrentDX8VertexBuffers[MAX_VERTEX_STREAMS];
unsigned							DX8Wrapper::CurrentDX8VertexBufferStrides[MAX_VERTEX_STREAMS];
IDirect3DIndexBuffer8*		DX8Wrapper::CurrentDX8IndexBuffer						= nullptr;
unsigned							DX8Wrapper::CurrentDX8IndexBufferBase					= 0;
...
void DX8Wrapper::Invalidate_Cached_Render_States()
{
	...
	memset(CurrentDX8VertexBuffers, 0xff, sizeof(CurrentDX8VertexBuffers));
	memset(CurrentDX8VertexBufferStrides, 0xff, sizeof(CurrentDX8VertexBufferStrides));
	CurrentDX8IndexBuffer=(IDirect3DIndexBuffer8*)-1;
	CurrentDX8IndexBufferBase=0xffffffff;
}
...
void DX8Wrapper::Release_Device()
{
	if (D3DDevice) {
		...
		_Set_DX8_Vertex_Buffer(0, nullptr, 0);	//release reference count on last rendered vertex buffer
		_Set_DX8_Index_Buffer(nullptr,0);	//release reference count on last rendered index buffer
...
void DX8Wrapper::Resize_And_Position_Window()
{
	...
	if ((rect.right-rect.left) != ResolutionWidth ||
		...
		if (!IsWindowed)
		{
			...
			// Borderless fullscreen: ensure WS_POPUP style and position at (0,0)
			...
			// MoveWindow is synchronous — avoids DWM race conditions that
			// cause SetWindowPos to intermittently leave the window offset.
			::MoveWindow(_Hwnd, 0, 0, screenW, screenH, TRUE);
			::SetWindowPos(_Hwnd, HWND_TOP, 0, 0, screenW, screenH,
...
#if defined(GENERALS_ONLINE_WINDOWED_FULLSCREEN)
	// D3D8 CreateDevice/Reset can reposition the window. Force it back to (0,0) fullscreen.
	if (ret && !IsWindowed)
	{
		int screenW = GetSystemMetrics(SM_CXSCREEN);
		int screenH = GetSystemMetrics(SM_CYSCREEN);
...
{
	DX8_THREAD_ASSERT();
	const double kEndMsPerTick = PerfGetFreqQuadPartAsMsMultiplier();
	LARGE_INTEGER _endQpcA, _endQpcB;
	#define END_PERF_BEGIN() do { if (kEndMsPerTick > 0.0) QueryPerformanceCounter(&_endQpcA); } while(0)
	#define END_PERF_END(tag) do { if (kEndMsPerTick > 0.0) { QueryPerformanceCounter(&_endQpcB); \
		PerfRecordLogicSub((tag), (double)(_endQpcB.QuadPart - _endQpcA.QuadPart) * kEndMsPerTick); } } while(0)

	END_PERF_BEGIN();
	DX8CALL(EndScene());
	END_PERF_END("Cl_End_EndScene");

	END_PERF_BEGIN();
	DX8WebBrowser::Render(0);
	END_PERF_END("Cl_End_Web");
	...
	END_PERF_END("Cl_End_Unbind");

	#undef END_PERF_BEGIN
	#undef END_PERF_END
}
...
{
	const unsigned desired_type = vb ? vb->Type() : BUFFER_TYPE_INVALID;
	IDirect3DVertexBuffer8* desired_dx_buffer = nullptr;
	unsigned desired_stride = 0;
	if (desired_type == BUFFER_TYPE_DX8) {
		desired_dx_buffer = const_cast<DX8VertexBufferClass*>(static_cast<const DX8VertexBufferClass*>(vb))->Get_DX8_Vertex_Buffer();
		desired_stride = vb->FVF_Info().Get_FVF_Size();
	}
	if (render_state.vertex_buffers[stream] == vb &&
		render_state.vertex_buffer_types[stream] == desired_type &&
		(stream != 0 || (render_state.vba_offset == 0 && render_state.vba_count == 0)) &&
		((render_state_changed&VERTEX_BUFFER_CHANGED) == 0) &&
		(desired_type == BUFFER_TYPE_DX8 || desired_type == BUFFER_TYPE_INVALID) &&
		CurrentDX8VertexBuffers[stream] == desired_dx_buffer &&
		CurrentDX8VertexBufferStrides[stream] == desired_stride)
	{
		return;
	}
...
{
	const unsigned desired_type = ib ? ib->Type() : BUFFER_TYPE_INVALID;
	IDirect3DIndexBuffer8* desired_dx_buffer = nullptr;
	if (desired_type == BUFFER_TYPE_DX8) {
		desired_dx_buffer = const_cast<DX8IndexBufferClass*>(static_cast<const DX8IndexBufferClass*>(ib))->Get_DX8_Index_Buffer();
	}
	if (render_state.index_buffer == ib &&
		render_state.index_buffer_type == desired_type &&
		render_state.index_base_offset == index_base_offset &&
		render_state.iba_offset == 0 &&
		((render_state_changed&INDEX_BUFFER_CHANGED) == 0) &&
		(desired_type == BUFFER_TYPE_DX8 || desired_type == BUFFER_TYPE_INVALID) &&
		CurrentDX8IndexBuffer == desired_dx_buffer &&
		CurrentDX8IndexBufferBase == index_base_offset)
	{
		return;
	}
	...
	unsigned short vertex_count)
{
	...
	{
		DX8_PERF_SCOPE("Cl_DX_SortVBCopy");
	...
	{
		DX8_PERF_SCOPE("Cl_DX_SortIBCopy");
	...
	unsigned short vertex_count)
{
	...
	const unsigned dirtyRenderState = render_state_changed & ~((unsigned)WORLD_IDENTITY | (unsigned)VIEW_IDENTITY);
	if (dirtyRenderState)
	{
...
void DX8Wrapper::Apply_Render_State_Changes()
{
	...
	if (render_state_changed&VERTEX_BUFFER_CHANGED) {
		...
		for (i=0;i<MAX_VERTEX_STREAMS;++i) {
			if (render_state.vertex_buffers[i]) {
				...
				_Set_DX8_Vertex_Buffer(i,nullptr,0);
			}
		}
	}
	if (render_state_changed&INDEX_BUFFER_CHANGED) {
		...
		if (render_state.index_buffer) {
			...
			_Set_DX8_Index_Buffer(nullptr,0);
		}
	}
...
{
	if (light) {
		if (render_state.LightEnable[index] && memcmp(&render_state.Lights[index], light, sizeof(*light)) == 0)
			return;

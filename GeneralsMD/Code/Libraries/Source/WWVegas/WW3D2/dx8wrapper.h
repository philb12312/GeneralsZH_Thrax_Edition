...
public:
	...
	*/
	...
	static void _Set_DX8_Vertex_Buffer(unsigned stream, IDirect3DVertexBuffer8* vertex_buffer, unsigned stride);
	static void _Set_DX8_Index_Buffer(IDirect3DIndexBuffer8* index_buffer, unsigned base_vertex_index);
...
protected:
	...
	*/
	...
	static IDirect3DVertexBuffer8*	CurrentDX8VertexBuffers[MAX_VERTEX_STREAMS];
	static unsigned						CurrentDX8VertexBufferStrides[MAX_VERTEX_STREAMS];
	static IDirect3DIndexBuffer8*		CurrentDX8IndexBuffer;
	static unsigned						CurrentDX8IndexBufferBase;
...
WWINLINE void DX8Wrapper::_Set_DX8_Transform(D3DTRANSFORMSTATETYPE transform, const D3DMATRIX& m)
{
	WWASSERT(transform<=D3DTS_WORLD);
	{
		...
		if (transform==D3DTS_WORLD && memcmp(&render_state.world,&m,sizeof(m)) != 0) {
			render_state_changed&=~(unsigned)WORLD_IDENTITY;
		}
		else if (transform==D3DTS_VIEW && memcmp(&render_state.view,&m,sizeof(m)) != 0) {
...
WWINLINE void DX8Wrapper::_Set_DX8_Vertex_Buffer(unsigned stream, IDirect3DVertexBuffer8* vertex_buffer, unsigned stride)
{
	WWASSERT(stream<MAX_VERTEX_STREAMS);
	if (CurrentDX8VertexBuffers[stream]==vertex_buffer && CurrentDX8VertexBufferStrides[stream]==stride) return;
	CurrentDX8VertexBuffers[stream]=vertex_buffer;
	CurrentDX8VertexBufferStrides[stream]=stride;
	DX8CALL(SetStreamSource(stream,vertex_buffer,stride));
	DX8_RECORD_VERTEX_BUFFER_CHANGE();
}

WWINLINE void DX8Wrapper::_Set_DX8_Index_Buffer(IDirect3DIndexBuffer8* index_buffer, unsigned base_vertex_index)
{
	if (CurrentDX8IndexBuffer==index_buffer && CurrentDX8IndexBufferBase==base_vertex_index) return;
	CurrentDX8IndexBuffer=index_buffer;
	CurrentDX8IndexBufferBase=base_vertex_index;
...
WWINLINE void DX8Wrapper::Set_Transform(D3DTRANSFORMSTATETYPE transform,const Matrix4x4& m)
{
	...
	case D3DTS_WORLD:
		{
			...
			if (memcmp(&render_state.world,&dxm,sizeof(dxm)) == 0 &&
				memcmp(&DX8Transforms[D3DTS_WORLD],&dxm,sizeof(dxm)) == 0 &&
				((render_state_changed&((unsigned)WORLD_CHANGED|(unsigned)WORLD_IDENTITY)) == 0)) {
				return;
			}
			render_state.world=dxm;
		}
	...
	case D3DTS_VIEW:
		{
			D3DMATRIX dxm=To_D3DMATRIX(m);
			if (memcmp(&render_state.view,&dxm,sizeof(dxm)) == 0 &&
				memcmp(&DX8Transforms[D3DTS_VIEW],&dxm,sizeof(dxm)) == 0 &&
				((render_state_changed&((unsigned)VIEW_CHANGED|(unsigned)VIEW_IDENTITY)) == 0)) {
				return;
			}
			render_state.view=dxm;
		}

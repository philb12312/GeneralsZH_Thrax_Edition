...
bool operator >=(const TempIndexStruct &l, const TempIndexStruct &r) { return l.z >= r.z; }
bool operator ==(const TempIndexStruct &l, const TempIndexStruct &r) { return l.z == r.z; }
static bool Render_States_Match_For_Sorted_Batch(const RenderStateStruct& a, const RenderStateStruct& b, int texture_count)
{
	if ((const unsigned&)a.shader != (const unsigned&)b.shader)
		return false;
	if (a.material != b.material)
		return false;
	if (std::memcmp(&a.world, &b.world, sizeof(a.world)) != 0 ||
	...
	for (int i = 0; i < 4; ++i) {
		if (a.LightEnable[i] != b.LightEnable[i])
			return false;
		if (a.LightEnable[i] && std::memcmp(&a.Lights[i], &b.Lights[i], sizeof(a.Lights[i])) != 0)
			return false;
	}
...
static inline void Expand_Sorted_Node_Vertex_Range(const SortingNodeStruct* state, unsigned& min_vertex, unsigned& max_vertex)
{
	const unsigned node_min = state->min_vertex_index;
	const unsigned node_max = state->min_vertex_index + state->vertex_count - 1;
	if (node_min < min_vertex) min_vertex = node_min;
	if (node_max > max_vertex) max_vertex = node_max;
}

static inline void Init_Sorted_Node_Vertex_Range(const SortingNodeStruct* state, unsigned& min_vertex, unsigned& max_vertex)
{
	min_vertex = state->min_vertex_index;
	max_vertex = state->min_vertex_index + state->vertex_count - 1;
}
...
*/
	...
	const int texture_count = DX8Wrapper::Get_Current_Caps()->Get_Max_Textures_Per_Pass();
	Init_Sorted_Node_Vertex_Range(overlapping_nodes[node_id], min_vertex_index, max_vertex_index);
	for (unsigned i=1;i<overlapping_polygon_count;++i) {
		const unsigned next_node_id = tis[i].idx;
		const bool matching_state = (node_id == next_node_id) ||
				Render_States_Match_For_Sorted_Batch(
					overlapping_nodes[node_id]->sorting_state,
					overlapping_nodes[next_node_id]->sorting_state,
					texture_count);
		if (!matching_state) {
			SortingNodeStruct* state=overlapping_nodes[node_id];
			...
			DX8Wrapper::Draw_Triangles(
				...
				count_to_render,
				min_vertex_index,
				max_vertex_index - min_vertex_index + 1);
		...
		if (node_id != next_node_id) {
			Expand_Sorted_Node_Vertex_Range(overlapping_nodes[next_node_id], min_vertex_index, max_vertex_index);
		}

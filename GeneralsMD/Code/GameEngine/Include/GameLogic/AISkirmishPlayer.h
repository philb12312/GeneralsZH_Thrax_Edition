...
protected:
	...
	Int sendIdleCombatTeamsToGuardPosition(const Coord3D *targetPos, const char *debugReason = nullptr, Int relatedPlayerIndex = -1);
	Int sendIdleCombatTeamsToPosition(const Coord3D *targetPos, const char *debugReason = nullptr, Int relatedPlayerIndex = -1);
	Int sendCombatTeamToMovePosition(Team *team, const Coord3D *targetPos, const char *debugReason = nullptr, Int relatedPlayerIndex = -1);
	...
	Int sendAllCombatTeamsToMovePosition(const Coord3D *targetPos, const char *debugReason = nullptr, Int relatedPlayerIndex = -1);
	...
	void clearAllCombatTeamCommandLocks();	///< Release the Team script-suppression lock set by human commands.
	...
	void serviceCoordinatedAttackBreakout();
...
protected:
	...
	UnsignedInt m_coordAttackBreakoutUntilFrame; ///< Coordinated attack breakout switches to attack-move after this frame.
	UnsignedInt m_frameNextCoordSkipLog;	///< Next frame on which the "GROUP: skip self-init" log is allowed (throttles spam).
	Int m_coordAttackLastLoggedLeadIdx;		///< Player index of last "joining lead" message we logged (dedupe). -1 = none.
	Coord3D m_coordAttackTarget;				///< Target position for the coordinated attack.
	Coord3D m_coordBreakoutPoint;			///< Egress point used to pull coordinated attacks clear of their base choke.
	...
	Bool m_coordAttackBreakoutPending;	///< True while a launched coordinated attack is still in move-only breakout mode.
	...
	Bool m_humanCommandIsAttack;			///< True when the active human command is an attack (use attack-move); false = guard mode.
	Int m_humanCommandAttackEnemyIndex;	///< Enemy player index for the human-commanded attack (for ping/chat display).

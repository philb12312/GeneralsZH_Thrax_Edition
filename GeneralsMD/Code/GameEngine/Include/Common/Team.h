...
private:
	...
	// TheSuperHackers @feature Bot-command suppression: while this frame is in the future, the
	// team's onIdle/onEnemySighted/onAllClear/onDestroyed and generic scripts are skipped so that
	// explicit human/bot orders are not overwritten by per-frame scripted behavior.
	UnsignedInt			m_commandLockUntilFrame;
...
public:
	...
	*/
	...
	/**
		TheSuperHackers @feature Bot command lock: suppress scripted re-issues until the given frame.
	*/
	void setCommandLockUntilFrame(UnsignedInt frame) { m_commandLockUntilFrame = frame; }
	UnsignedInt getCommandLockUntilFrame() const { return m_commandLockUntilFrame; }

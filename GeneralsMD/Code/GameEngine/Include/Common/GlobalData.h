...
public:
  ...
  Int					m_playStats;									///< Int whether we want to log play stats or not, if <= 0 then we don't log
	Bool				m_enableRuntimeProfiling;	///< Manual switch for release-active detailed runtime profiling/logging; cheap summary-only perf files are handled separately.
	Bool				m_benchmarkMode;						///< Automated benchmark mode requested from the command line.
	Bool				m_benchmarkStartupPending;	///< Benchmark save still needs to be loaded.
	Bool				m_benchmarkWaitingForSettle;	///< Benchmark save is loaded and the camera is still settling at max zoom.
	Bool				m_benchmarkSamplingActive;	///< Timed benchmark sampling window is currently active.
	UnsignedInt	m_benchmarkSamplingStartMs;	///< Wall-clock start time of the timed benchmark run.
	UnsignedInt	m_benchmarkSettleStartMs;	///< Wall-clock time when benchmark max-zoom settling began.
	UnsignedInt	m_benchmarkStableSinceMs;	///< Wall-clock time when the max-zoom camera first appeared settled.
	Real				m_benchmarkLastCameraHeight;	///< Last observed camera height above ground while waiting for max zoom settle.
	AsciiString m_benchmarkSaveFile;				///< Save file leaf to load from the user's Save directory.

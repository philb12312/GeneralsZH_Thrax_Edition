	...
	NGMP_OnlineServicesManager::GetInstance()->GetHTTPManager()->SendPOSTRequest(strURI.c_str(), EIPProtocolVersion::DONT_CARE, mapHeaders, strPostData.c_str(), [=](bool bSuccess, int statusCode, std::string strBody, HTTPRequest* pReq)
		{
			...
			try
			{
				...
				{
					NetworkLog(ELogVerbosity::LOG_RELEASE, "VERSION CHECK: Needs Update (treating as OK for custom build)");

					// CUSTOM BUILD: Still allow connection even if server says needs update.
					// The important part is that the CRC was sent to the server above.
					fnCallback(true, false);
				}
				else
				{
...
void NGMP_OnlineServicesManager::InitSentry()
{
	...
	// Public fork: no project-specific Sentry endpoint is configured by default.
	// Set a DSN here only for private builds that own the target Sentry project.

...
static int ClampDetectedMonitorFps(int hz)
{
	if (hz <= 1)
	{
		return 0;
	...
	return (hz > 165) ? 165 : hz;
}

static int GetDx8DisplayModeFps()
{
	IDirect3DDevice8* const device = DX8Wrapper::_Get_D3D_Device8();
	if (device != nullptr)
	{
		D3DDISPLAYMODE mode = {};
		if (SUCCEEDED(device->GetDisplayMode(&mode)))
		{
			return ClampDetectedMonitorFps((int)mode.RefreshRate);
		}
	}
...
static int GetWindowMonitorFps()
{
	if (ApplicationHWnd != nullptr)
	{
		...
		const HMONITOR monitor = MonitorFromWindow(ApplicationHWnd, MONITOR_DEFAULTTONEAREST);
		if (monitor != nullptr && GetMonitorInfo(monitor, &monitorInfo))
		{
			...
			{
				return ClampDetectedMonitorFps((int)devMode.dmDisplayFrequency);
			}
		}
	}
...
int GetDefaultMonitorFps()
{
	if (const int hz = GetDx8DisplayModeFps(); hz > 0)
	{
		return hz;
	}

	if (const int hz = GetWindowMonitorFps(); hz > 0)
	{
		return hz;
	}
	...
	{
		if (const int hz = ClampDetectedMonitorFps((int)devMode.dmDisplayFrequency); hz > 0)
		{
			return hz;
		}
	}
...
{
	// CUSTOM BUILD: Always allow maximum zoom-out regardless of lobby settings
	return 1000.f;
}
...
void GenOnlineSettings::Load(void)
{
	...
	if (bApplyDefaults)
	{
		...
		m_Render_FramerateLimit_FPSVal = GetDefaultMonitorFps(); // TheSuperHackers @mod Default to monitor refresh rate (capped at 165)

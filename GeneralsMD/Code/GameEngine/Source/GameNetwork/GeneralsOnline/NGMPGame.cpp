...
void NGMPGame::launchGame(void)
{
	...
	if (pOnlineServicesMgr != nullptr)
	{
		...
		if (serviceConf.use_default_config)
		{
			...
			SetFrameGroupingCap(GENERALS_ONLINE_DEFAULT_FRAME_GROUPING_CAP);
		}
		else
		{
			...
			SetFrameGroupingCap((int)(serviceConf.frame_grouping_frames * (float)(1000.f / GENERALS_ONLINE_HIGH_FPS_LIMIT)));
		}
	}

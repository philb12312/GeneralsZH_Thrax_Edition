...
void PopulateColorComboBox(Int comboBox, GameWindow *comboArray[], GameInfo *myGame, Bool isObserver)
{
	...
	GameSlot *currentSlot = myGame ? myGame->getSlot(comboBox) : nullptr;
	// Shared-color armies are only meaningful in actual multiplayer setup.
	// If we hide already-taken colors here, other players can neither see nor
	// select the existing shared color group in the lobby UI.
	Bool allowTakenColors = myGame && myGame->isInGame() && myGame->isMultiPlayer()
		&& currentSlot && currentSlot->isHuman();
	...
	for (Int c=0; c<numColors; ++c)
	{
		...
		if (!def || (!allowTakenColors && availableColors[c] == false))
			continue;

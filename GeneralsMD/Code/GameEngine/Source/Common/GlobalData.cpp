...
GlobalData::GlobalData()
{
  ...
  m_showAudioLocations = FALSE;
	...
	m_playStats = -1;
	m_enableRuntimeProfiling = FALSE;
	m_benchmarkMode = FALSE;
	m_benchmarkStartupPending = FALSE;
	m_benchmarkWaitingForSettle = FALSE;
	m_benchmarkSamplingActive = FALSE;
	m_benchmarkSamplingStartMs = 0;
	m_benchmarkSettleStartMs = 0;
	m_benchmarkStableSinceMs = 0;
	m_benchmarkLastCameraHeight = -1.0f;
	m_benchmarkSaveFile.clear();
...
#define GENERALSMD_104_EAAPP_EXE_CRC 0xc4181eb9u
...
#else
	{
		...
		// Use fopen() since TheFileSystem->openFile() cannot handle absolute paths with drive letters.
		{
			...
			strncpy(altPath, buffer, _MAX_PATH);
			altPath[_MAX_PATH - 1] = '\0';
			char* lastSlash = strrchr(altPath, '\\');
			if (!lastSlash) lastSlash = strrchr(altPath, '/');
			if (lastSlash)
			{
				*(lastSlash + 1) = '\0';
			...
			size_t amtRead = 0;
			while ( (amtRead = fread(crcBlock, 1, blockSize, altFile)) > 0 )
			{

...
void Team::updateState()
{
	...
	// TheSuperHackers @feature Bot command lock: while human-commanded, suppress ALL scripted
	// state reactions (including onCreate) so the bot's explicit orders are not clobbered.
	// We still clear m_created so the flag doesn't stick indefinitely.
	const Bool commandLocked = isCommandLocked();

	if (m_created)
	{
		m_created = false;
		// Run the on create script, if any.
		if (!commandLocked && !pInfo->m_scriptOnCreate.isEmpty())
		{
...
// ------------------------------------------------------------------------
// TheSuperHackers @feature Bot command lock
Bool Team::isCommandLocked() const
{
	if (m_commandLockUntilFrame == 0)
		return false;
	if (TheGameLogic == nullptr)
		return false;
	return TheGameLogic->getFrame() < m_commandLockUntilFrame;
}

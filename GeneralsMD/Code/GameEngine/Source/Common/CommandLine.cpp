...
static Bool isValidBenchmarkSaveLeaf(const AsciiString& saveLeaf)
{
...
Int parseBenchmarkMode(char *args[], int num)
{
	if (num > 1)
	{
		AsciiString saveLeaf = args[1];
		if (!isValidBenchmarkSaveLeaf(saveLeaf))
		{
			DEBUG_LOG(("Ignoring invalid benchmark save file '%s'", saveLeaf.str()));
			return 2;
		}
		...
		TheWritableGlobalData->m_benchmarkMode = TRUE;
		TheWritableGlobalData->m_benchmarkStartupPending = TRUE;
...
Int parseProfiling(char *args[], int)
{
	TheWritableGlobalData->m_enableRuntimeProfiling = TRUE;

	return 1;
}

Int parseNoProfiling(char *args[], int)
{
	TheWritableGlobalData->m_enableRuntimeProfiling = FALSE;
...
static CommandLineParam paramsForEngineInit[] =
{
	...
	{ "benchmark", parseBenchmarkMode },
	{ "-profiling", parseProfiling },
	{ "-noProfiling", parseNoProfiling },

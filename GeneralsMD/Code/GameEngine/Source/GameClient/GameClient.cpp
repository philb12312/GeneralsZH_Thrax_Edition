...
namespace
{
	constexpr DWORD kBenchmarkZoomSettleMs = 1000u;
	constexpr DWORD kBenchmarkMaxSettleMs = 30000u;
	constexpr Real kBenchmarkHeightStableEpsilon = 0.25f;

	static Bool isBenchmarkSaveLeafValid(const AsciiString& saveLeaf)
	{
		return saveLeaf.isEmpty() == FALSE
			&& strchr(saveLeaf.str(), '\\') == nullptr
			&& strchr(saveLeaf.str(), '/') == nullptr
			&& strchr(saveLeaf.str(), ':') == nullptr;
	}

	static Bool isBenchmarkStartupReady()
	{
		...
		if (TheShell && !TheShell->isAnimFinished())
		{
			return FALSE;
		}

		if (TheTransitionHandler && !TheTransitionHandler->isFinished())
		{
			return FALSE;
		}

		if (TheGameLogic && (TheGameLogic->isLoadingMap() || TheGameLogic->isLoadingSave()))
		{
			return FALSE;
		}

		if (TheGlobalData->m_loadScreenRender)
		{
			return FALSE;
		}
	...
	static void stopBenchmarkMode(const char* reason)
	{
		DEBUG_LOG(("Benchmark mode aborted: %s", reason ? reason : "unknown reason"));
		TheWritableGlobalData->m_benchmarkMode = FALSE;
		...
		TheWritableGlobalData->m_benchmarkSamplingStartMs = 0;
		TheWritableGlobalData->m_benchmarkSettleStartMs = 0;
		TheWritableGlobalData->m_benchmarkStableSinceMs = 0;
		TheWritableGlobalData->m_benchmarkLastCameraHeight = -1.0f;
		TheGameEngine->setQuitting(TRUE);
	}

	static Bool startBenchmarkSaveLoad()
	{
		AsciiString saveLeaf = TheGlobalData->m_benchmarkSaveFile;
		if (!isBenchmarkSaveLeafValid(saveLeaf))
		{
			stopBenchmarkMode("invalid benchmark save file name");
			return FALSE;
		}

		if (!saveLeaf.endsWithNoCase(".sav"))
		{
			saveLeaf.concat(".sav");
		}

		if (!TheGameState->doesSaveGameExist(saveLeaf))
		{
			DEBUG_LOG(("Benchmark save '%s' was not found in '%s'", saveLeaf.str(), TheGameState->getSaveDirectory().str()));
			stopBenchmarkMode("benchmark save file not found");
			return FALSE;
		}
		...
		try
		{
			TheGameState->getSaveGameInfoFromFile(TheGameState->getFilePathInSaveDirectory(saveLeaf), &saveGameInfo);
		}
		catch (...)
		{
			stopBenchmarkMode("failed to read benchmark save metadata");
			return FALSE;
		}
		...
		gameInfo.filename = saveLeaf;
		gameInfo.saveGameInfo = saveGameInfo;

		TheWritableGlobalData->m_benchmarkSaveFile = saveLeaf;
		TheWritableGlobalData->m_benchmarkStartupPending = FALSE;
		TheWritableGlobalData->m_benchmarkWaitingForSettle = FALSE;
		TheWritableGlobalData->m_benchmarkSamplingActive = FALSE;
		TheWritableGlobalData->m_benchmarkSamplingStartMs = 0;
		...
		{
			TheTransitionHandler->remove("MainMenuDefaultMenuLogoFade", TRUE);
		}
		...
		{
			stopBenchmarkMode("benchmark save load failed");
			return FALSE;
		}
		...
		TheWritableGlobalData->m_benchmarkWaitingForSettle = TRUE;
		TheWritableGlobalData->m_benchmarkSettleStartMs = timeGetTime();

		return TRUE;
	}

	static void updateBenchmarkZoomSettle()
	{
		if (!TheGlobalData->m_benchmarkWaitingForSettle)
		{
			return;
		}

		const DWORD nowMs = timeGetTime();
		if (TheGlobalData->m_benchmarkSettleStartMs > 0
			&& nowMs - TheGlobalData->m_benchmarkSettleStartMs >= kBenchmarkMaxSettleMs)
		{
			stopBenchmarkMode("benchmark camera settle timed out");
			return;
		}

		if (!TheGameLogic->isInGame() || TheTacticalView == nullptr)
		{
			return;
		}

		if (TheGlobalData->m_benchmarkLastCameraHeight < 0.0f)
		{
		...
		const Real currentHeight = TheTacticalView->getCurrentHeightAboveGround();
		...
		if (TheGlobalData->m_benchmarkLastCameraHeight < 0.0f)
		{
			TheWritableGlobalData->m_benchmarkLastCameraHeight = currentHeight;
			TheWritableGlobalData->m_benchmarkStableSinceMs = 0;
			return;
		}

		const Real heightDelta = fabs(currentHeight - TheGlobalData->m_benchmarkLastCameraHeight);
		if (heightDelta <= kBenchmarkHeightStableEpsilon)
		{
			if (TheGlobalData->m_benchmarkStableSinceMs == 0)
			{
				TheWritableGlobalData->m_benchmarkStableSinceMs = nowMs;
			}
			else if (nowMs - TheGlobalData->m_benchmarkStableSinceMs >= kBenchmarkZoomSettleMs)
			{
				TheWritableGlobalData->m_benchmarkWaitingForSettle = FALSE;
				TheWritableGlobalData->m_benchmarkSamplingActive = TRUE;
				TheWritableGlobalData->m_benchmarkSamplingStartMs = nowMs;
				TheWritableGlobalData->m_benchmarkSettleStartMs = 0;
			}
		}
		else
...
// --- Per-subsystem perf instrumentation (Cl_* tags) ---------------------------
// Feeds into the same PerfRecordLogicSub bucket used by Lg_* logic tags, so the
// same SUB-BREAKDOWN block in the perf log shows both client and logic costs.
// This is the only way to see WHERE render time goes. Keep it lightweight:
// QueryPerformanceCounter is sub-microsecond on modern Windows.
...
void GameClient::update()
{
	...
	const Bool startBenchmarkAfterFrame = TheGlobalData->m_benchmarkMode
		&& TheGlobalData->m_benchmarkStartupPending
		&& isBenchmarkStartupReady();

	const double kMsPerTick = PerfGetFreqQuadPartAsMsMultiplier();
	LARGE_INTEGER _qpcA, _qpcB;
	#define CL_PERF_BEGIN() QueryPerformanceCounter(&_qpcA)
	#define CL_PERF_END(tag) do { QueryPerformanceCounter(&_qpcB); \
	...
	// create the FRAME_TICK message
	{
		...
		CL_PERF_END("Cl_FrameMsg");
	}
	static Bool playSizzle = FALSE;
	if (TheGlobalData->m_benchmarkMode)
	{
		updateBenchmarkZoomSettle();
	}
...
//		TheParticleSystemManager->update();
	...
	if (startBenchmarkAfterFrame)
	{
		startBenchmarkSaveLoad();
		return;
	}

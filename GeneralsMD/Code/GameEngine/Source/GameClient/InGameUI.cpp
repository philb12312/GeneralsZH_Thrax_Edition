...
void InGameUI::postDraw()
{
	...
	if (m_messagesOn)
	{
		...
		// TheSuperHackers @feature community - Center chat at lower 1/3 of screen
		// Messages draw bottom-to-top, starting at ~67% screen height
		Int screenW = TheDisplay ? (Int)TheDisplay->getWidth() : 800;
		Int screenH = TheDisplay ? (Int)TheDisplay->getHeight() : 600;
		// Start position: lower third, about 2-3cm above the bottom command bar
		// The command bar is roughly 120px tall at 768p, scale proportionally
		Int bottomMargin = screenH / 5; // ~20% from bottom
		const Int chatLiftPx = 50;
		Int startY = screenH - bottomMargin - chatLiftPx;

		// First pass: count total height of visible messages so we can start from the bottom
		Int totalHeight = 0;
		Int visibleCount = 0;
		for (i = MAX_UI_MESSAGES - 1; i >= 0; i--)
		{
		...
		// Draw from the top of the message block downward
		...
		for (i = MAX_UI_MESSAGES - 1; i >= 0; i--)
		{
			...
			if (m_uiMessages[i].displayString)
			{
				...
				Int textW = 0, textH = 0;
				m_uiMessages[i].displayString->getSize(&textW, &textH);

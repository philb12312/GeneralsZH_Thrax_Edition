...
static const LookupListRec GameMessageMetaTypeNames[] =
{
	...
	{ "VIEW_HERO",													      GameMessage::MSG_META_VIEW_HERO },
...
/*static */ void MetaMap::generateMetaMap()
{
	...
	{
		// TheSuperHackers @feature - F1 centers camera on hero without changing selection
		MetaMapRec* map = TheMetaMap->getMetaMapRec(GameMessage::MSG_META_VIEW_HERO);
		if (map->m_key == MK_NONE) {
			map->m_key = MK_F1;
			map->m_transition = DOWN;
			map->m_modState = NONE;
			...
			map->m_description = TheGameText->FETCH_OR_SUBSTITUTE("GUI:ViewHeroDescription", L"Center camera on hero unit");
			map->m_displayName = TheGameText->FETCH_OR_SUBSTITUTE("GUI:ViewHero", L"View Hero");
		}
	}
	{

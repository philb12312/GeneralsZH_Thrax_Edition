...
GameMessageDisposition LookAtTranslator::translateGameMessage(const GameMessage* msg)
{
	...
	case GameMessage::MSG_RAW_MOUSE_RIGHT_BUTTON_DOWN:
	{
		...
		// Don't start RMB scroll if mouse is over the radar — let the radar handle right-click drag.
		Coord3D radarTestWorld;
		ICoord2D radarTestPixel = msg->getArgument(0)->pixel;
		Bool mouseOverRadar = (TheRadar != nullptr && TheRadar->screenPixelToWorld(&radarTestPixel, &radarTestWorld));

		if (!TheInGameUI->isSelecting() && !m_isScrolling && !mouseOverRadar)
		{

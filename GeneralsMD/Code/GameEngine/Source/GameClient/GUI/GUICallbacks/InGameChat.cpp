...
static void NotifyPlayersOfChatMessage(const Player *sender, const UnicodeString &text, Int playerMask)
{
	...
	for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i)
	{
		...
		if (player)
		{
			player->onChatMessage(sender, text, playerMask);
		}
	}
}

static const Player *FindChatPlayerForSlot(Int slotNum)
{
	if (ThePlayerList == nullptr)
	{
		return nullptr;
	...
	Int playerSlot = slotNum;
	if (TheGameInfo)
	{
		playerSlot = TheGameInfo->getSharedColorOwnerSlot(slotNum);
	}

	AsciiString playerName;
	playerName.format("player%d", playerSlot);
	return ThePlayerList->findPlayerWithNameKey(TheNameKeyGenerator->nameToKey(playerName));
}
...
void ToggleInGameChat( Bool immediate )
{
	...
	// TheSuperHackers @feature community Allow chat in all game types (singleplayer, skirmish, coop)
	// Original code blocked chat in non-multiplayer games.
		...
		else
		{
			if (chatTextEntry)
			{
				...
				if (!msg.isEmpty() && !handleInGameSlashCommands(msg))
				{
					...
					{
						const GameSlot *slot = TheGameInfo ? TheGameInfo->getConstSlot(i) : nullptr;
						if (slot && !slot->isOccupied())
						{
							continue;
						}

						const Player *player = FindChatPlayerForSlot(i);
					...
					// TheSuperHackers @feature community
					// If network is available (multiplayer), use normal send path.
					// Otherwise (singleplayer/skirmish/coop), display locally in player color.
					if (TheNetwork != nullptr)
					{
						TheNetwork->sendChat(msg, playerMask);
					}
					else if (ExecuteMoneyTransferChatCommand(localPlayer, msg))
					{
						// Money commands are handled locally in offline games and should not be echoed as normal chat.
					}
					else
					{
						// Display the player's own message locally with their player color
						UnicodeString displayMsg;
						UnicodeString localDisplayName = localPlayer ? localPlayer->getPlayerDisplayName() : UnicodeString(L"Player");
						displayMsg.format(L"%ls: %ls", localDisplayName.str(), msg.str());

						if (localPlayer)
						{
							RGBColor playerRGB;
							playerRGB.setFromInt(localPlayer->getPlayerColor() & 0x00FFFFFF);
							TheInGameUI->messageNoFormat(&playerRGB, displayMsg);
						}
						else
						{
							TheInGameUI->messageNoFormat(displayMsg);
						}
						// Dispatch the chat message to allied AI bots (offline only, preserves determinism).
						NotifyPlayersOfChatMessage(localPlayer, msg, playerMask);
					}
				}
				GadgetTextEntrySetText( chatTextEntry, UnicodeString::TheEmptyString );

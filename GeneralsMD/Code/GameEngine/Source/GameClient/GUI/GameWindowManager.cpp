...
namespace
{
	class GameWindowPerfScope
	{
	public:
		explicit GameWindowPerfScope(const char *tag) : m_token(PerfBeginScopedLogicSub(tag)) {}
		~GameWindowPerfScope() { PerfEndScopedLogicSub(m_token); }
...
void GameWindowManager::winRepaint()
{
	...
	// draw below windows
	{
		GameWindowPerfScope perfScope("Cl_UI_Win_Below");
	...
	{
		GameWindowPerfScope perfScope("Cl_UI_Win_Main");
		for (window = m_windowTail; window; window = next)
		{
			next = window->m_prev;
	...
	// draw above windows
	{
		GameWindowPerfScope perfScope("Cl_UI_Win_Above");
		for (window = m_windowTail; window; window = next)
		{
			next = window->m_prev;
	...
	if (TheTransitionHandler)
	{
		GameWindowPerfScope perfScope("Cl_UI_Win_Trans");

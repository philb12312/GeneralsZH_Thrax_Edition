...
// Perf instrumentation hooks (defined in GameEngine.cpp)
...
static void checkForDuplicateColors(GameInfo* game)
{
	...
	// Duplicate concrete colors are intentionally supported as shared-control
	// groups. Random or invalid colors are resolved by populateRandomSideAndColor.
}
...
static void populateRandomStartPosition(GameInfo* game)
{
	...
	for (i = 0; i < MAX_SLOTS; ++i)
	{
		...
		if (!slot || !slot->isOccupied() || slot->getPlayerTemplate() == PLAYERTEMPLATE_OBSERVER || game->isSharedColorSecondarySlot(i))
			continue;
	...
	for (i = 0; i < MAX_SLOTS; ++i)
	{
		...
		if (slot->isOccupied() && slot->getPlayerTemplate() != PLAYERTEMPLATE_OBSERVER && !game->isSharedColorSecondarySlot(i))
			++numPlayersInGame;
	}
	for (i = 0; i < MAX_SLOTS; ++i)
	{
...
void GameLogic::startNewGame(Bool loadingSaveGame)
{
	...
	if (game)
	{
		...
		if (localSlotNum >= 0)
		{
			localSharedOwnerSlot = game->getSharedColorOwnerSlot(localSlotNum);
		}
		else
		{
			DEBUG_LOG(("GameLogic::startNewGame - local slot unavailable while populating multiplayer sides"));
		}
	...
	if (game)
		game->normalizeSharedColorSlots();
	...
	if (game)
	{
		...
		for (int i = 0; i < MAX_SLOTS; ++i)
		{
			...
			if (game->isSharedColorSecondarySlot(i))
				continue;
			...
			for (int j = 0; j < MAX_SLOTS; ++j)
			{
				...
				if (i == j || !teamSlot->isOccupied() || game->isSharedColorSecondarySlot(j))
					continue;
			...
			d.setBool(TheKey_multiplayerIsLocal, slot->isHuman() && i == localSharedOwnerSlot);
			if (i == localSharedOwnerSlot)
			{
				localSlot = i;
			}
	...
	if (game)
	{
		...
		{
			slot = game->getSlot(localSlot);
		}
		else
		{
			DEBUG_LOG(("GameLogic::startNewGame - local slot unresolved, using default initial camera"));
		}
...
void GameLogic::update()
{
	...
	// -------- Fine-grained subsystem timing (release-active) --------
	const double kMsPerTick = PerfGetFreqQuadPartAsMsMultiplier();
	LARGE_INTEGER _qpcA, _qpcB;
	#define PERF_SUB_BEGIN() QueryPerformanceCounter(&_qpcA)
	#define PERF_SUB_END(tag) do { QueryPerformanceCounter(&_qpcB); \
...

	{
		...
		PERF_SUB_END("Lg_Stores");
	}

	{
		PERF_SUB_BEGIN();
		TheVictoryConditions->UPDATE();
		PERF_SUB_END("Lg_Victory");
	}

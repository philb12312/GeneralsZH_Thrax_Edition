...
enum { REPEATED_NO_PATH_RETRY_FRAMES = LOGICFRAMES_PER_SECOND };
enum { ASYNC_PATH_RESULT_MAX_AGE_FRAMES = LOGICFRAMES_PER_SECOND * 2 };
static const Real ASYNC_PATH_RESULT_MAX_MOVE = PATHFIND_CELL_SIZE_F * 6.0f;

static Bool isSamePathTarget(const Coord3D *ourPos, const Coord3D *prevTargetPos, const Coord3D *curTargetPos)
{
	Coord3D diff;
	...
	return diff.x * diff.x + diff.y * diff.y <= toleranceSqr;
}

static Int debugCountPathNodes(Path *path)
{
	Int count = 0;
...
static const char *debugPathCellTypeName(const PathfindCell *cell)
{
	if (cell == nullptr)
		return "null";
	...
	UpdateModule( thing, moduleData )
{
	...
	m_path = nullptr;
	m_pathRequestSerial = 1;
	m_asyncPathRequestSerial = 0;
...
//-------------------------------------------------------------------------------------------------
Bool AIUpdateInterface::buildAsyncPathfindJob( AsyncPathfindJob &job )
{
	if (!m_waitingForPath || hasAsyncPathfindInFlight())
		return FALSE;

	Object *obj = getObject();
	if (!obj || obj->isEffectivelyDead() || obj->isKindOf(KINDOF_PROJECTILE))
		return FALSE;

	if (!isDoingGroundMovement() || canComputeQuickPath())
		return FALSE;

	if (obj->getLayer() != LAYER_GROUND || m_locomotorSet.getValidSurfaces() == 0)
		return FALSE;
	...
	radius = REAL_TO_INT_FLOOR(diameter / PATHFIND_CELL_SIZE_F + 0.3f);
	centerInCell = FALSE;
	if (radius == 0)
		radius++;
	if (radius & 1)
	...
	Coord3D goalPos = m_requestedDestination;
	Coord3D secondaryTarget;
	secondaryTarget.zero();
	Coord3D tertiaryTarget;
	tertiaryTarget.zero();
	AsyncPathfindMode mode = ASYNC_PATH_MODE_MOVE;
	Real attackRange = 0.0f;
	Real safeRadius = 0.0f;
	Bool requireAttackLine = FALSE;
	Bool provisionalAllowed = TRUE;
	Bool isFinalGoal = m_isFinalGoal;

	if (m_isSafePath)
	{
		mode = ASYNC_PATH_MODE_SAFE;
		provisionalAllowed = FALSE;
		isFinalGoal = FALSE;
		secondaryTarget.set(-1000.0f, -1000.0f, 0.0f);
		tertiaryTarget = secondaryTarget;
		Object *repulsor = TheGameLogic->findObjectByID(m_repulsor1);
		if (repulsor)
			secondaryTarget = *repulsor->getPosition();
		repulsor = TheGameLogic->findObjectByID(m_repulsor2);
		if (repulsor)
			tertiaryTarget = *repulsor->getPosition();
		safeRadius = obj->getVisionRange() + TheAI->getAiData()->m_repulsedDistance;
		goalPos = *obj->getPosition();
	}
	else if (m_isAttackPath || m_requestedVictimID != INVALID_ID)
	{
		Object *victim = m_requestedVictimID != INVALID_ID ? TheGameLogic->findObjectByID(m_requestedVictimID) : nullptr;
		Weapon *weapon = obj->getCurrentWeapon();
		if (!weapon)
			return FALSE;

		PathfindLayerEnum victimLayer = LAYER_GROUND;
		Coord3D localVictimPos = m_requestedDestination;
		if (victim != nullptr)
		{
			if (victim->isKindOf(KINDOF_BRIDGE))
			{
				...
				Real distSqr1 = ThePartitionManager->getDistanceSquared(obj, &info.attackPoint1, FROM_BOUNDINGSPHERE_3D);
				Real distSqr2 = ThePartitionManager->getDistanceSquared(obj, &info.attackPoint2, FROM_BOUNDINGSPHERE_3D);
				localVictimPos = distSqr2 < distSqr1 ? info.attackPoint2 : info.attackPoint1;
			}
			else
			{
		...
		{
			weapon->computeApproachTarget(obj, victim, &localVictimPos, 0, goalPos);
			if (getCurLocomotor())
				getCurLocomotor()->setNoSlowDownAsApproachingDest(TRUE);
			mode = ASYNC_PATH_MODE_MOVE;
			isFinalGoal = TRUE;
		}
		else
		{
			goalPos = localVictimPos;
			mode = ASYNC_PATH_MODE_ATTACK;
			attackRange = (Real)(weapon->getAttackDistance(obj, victim, &localVictimPos) + 3 * PATHFIND_CELL_SIZE);
			requireAttackLine = isDoingGroundMovement() && (victim == nullptr || !victim->isSignificantlyAboveTerrain());
			isFinalGoal = FALSE;
		}
	}
	else if (m_isBlockedAndStuck && m_path && m_path->getLastNode())
	{
	...
	if (goalPos.x < extent.lo.x)
		goalPos.x = extent.lo.x;
	else if (goalPos.x > extent.hi.x)
		goalPos.x = extent.hi.x;
	if (goalPos.y < extent.lo.y)
		goalPos.y = extent.lo.y;
	else if (goalPos.y > extent.hi.y)
		goalPos.y = extent.hi.y;

	job.objectID = obj->getID();
	job.requestSerial = m_pathRequestSerial;
	job.submitFrame = TheGameLogic->getFrame();
	job.mode = mode;
	job.from = *obj->getPosition();
	job.to = goalPos;
	job.secondaryTarget = secondaryTarget;
	job.tertiaryTarget = tertiaryTarget;
	job.originalDestination = mode == ASYNC_PATH_MODE_MOVE ? m_requestedDestination : goalPos;
	job.acceptableSurfaces = m_locomotorSet.getValidSurfaces();
	job.layer = obj->getLayer();
	job.destinationLayer = TheTerrainLogic->getLayerForDestination(&goalPos);
	job.attackRange = attackRange;
	job.safeRadius = safeRadius;
	job.radius = radius;
	job.centerInCell = centerInCell;
	job.isCrusher = obj->getCrusherLevel() > 0;
	job.canPathThroughUnits = canPathThroughUnits();
	job.isFinalGoal = isFinalGoal;
	job.ultraAccurate = getCurLocomotor() && getCurLocomotor()->isUltraAccurate();
	job.preferClosest = mode != ASYNC_PATH_MODE_MOVE;
	job.requireAttackLine = requireAttackLine;
	job.provisionalAllowed = provisionalAllowed;
	job.ignoreObstacleID = getIgnoredObstacleID();

	m_asyncPathRequestSerial = m_pathRequestSerial;
	return TRUE;
}

//-------------------------------------------------------------------------------------------------
Bool AIUpdateInterface::isAsyncPathfindResultCurrent( const AsyncPathfindResult &result ) const
{
	return m_pathRequestSerial == result.requestSerial && m_asyncPathRequestSerial == result.requestSerial;
}

//-------------------------------------------------------------------------------------------------
Bool AIUpdateInterface::applyAsyncPathfindResult( const AsyncPathfindResult &result )
{
	if (!isAsyncPathfindResultCurrent(result))
		return FALSE;

	if (!result.provisional && TheGameLogic->getFrame() > result.submitFrame + ASYNC_PATH_RESULT_MAX_AGE_FRAMES)
	{
		DEBUG_LOG(("%d AsyncPathResult stale obj=%d tmpl='%s' req=%u age=%u from=(%.3f,%.3f) cur=(%.3f,%.3f) dest=(%.3f,%.3f) provisional=%d partial=%d",
			TheGameLogic->getFrame(),
			getObject()->getID(),
			getObject()->getTemplate()->getName().str(),
			result.requestSerial,
			TheGameLogic->getFrame() - result.submitFrame,
			result.from.x, result.from.y,
			getObject()->getPosition()->x, getObject()->getPosition()->y,
			result.originalDestination.x, result.originalDestination.y,
			result.provisional ? 1 : 0,
			result.partial ? 1 : 0));
		m_asyncPathRequestSerial = 0;
		return FALSE;
	}
	...
	{
		const Coord3D *currentPos = getObject()->getPosition();
		Real dx = currentPos->x - result.from.x;
		Real dy = currentPos->y - result.from.y;
		if (dx * dx + dy * dy > ASYNC_PATH_RESULT_MAX_MOVE * ASYNC_PATH_RESULT_MAX_MOVE)
		{
			DEBUG_LOG(("%d AsyncPathResult moved-too-far obj=%d tmpl='%s' req=%u move=%.3f limit=%.3f from=(%.3f,%.3f) cur=(%.3f,%.3f) dest=(%.3f,%.3f)",
				TheGameLogic->getFrame(),
				getObject()->getID(),
				getObject()->getTemplate()->getName().str(),
				result.requestSerial,
				sqrt(dx * dx + dy * dy),
				ASYNC_PATH_RESULT_MAX_MOVE,
				result.from.x, result.from.y,
	...
	m_retryPath = result.partial;
	if (!result.success)
	{
		if (!result.provisional)
		{
			m_asyncPathRequestSerial = 0;
			// Keep the async fast path for successful jobs, but fall back to the original
			// synchronous solver when the worker returns a hard failure. The legacy solver
			// still handles closest-path/tunneling edge cases that can otherwise leave units
			// pinned against cliffs, ramps, or dense obstacle funnels with pathNodes=0.
			m_waitingForPath = TRUE;
			doPathfind(TheAI->pathfinder()->getPathfindServices());
			if (!m_waitingForPath)
				wakeUpNow();
			return TRUE;
		}
	...
	if (result.success && !result.pathCells.empty())
	{
		theNewPath = newInstance(Path);
		const Real cellOffset = result.centerInCell ? 0.5f : 0.05f;

		Coord3D pos = *getObject()->getPosition();
		theNewPath->prependNode(&pos, getObject()->getLayer());

		UnsignedInt firstCell = result.pathCells.size() > 1 ? 1 : 0;
		for (UnsignedInt i = firstCell; i < result.pathCells.size(); ++i)
		{
			PathfindLayerEnum layer = result.pathLayers.size() > i ? result.pathLayers[i] : LAYER_GROUND;
			const ICoord2D &cell = result.pathCells[i];
			Coord3D nodePos;
			if (layer == LAYER_INVALID)
				layer = LAYER_GROUND;
			nodePos.x = ((Real)cell.x + cellOffset) * PATHFIND_CELL_SIZE_F;
			nodePos.y = ((Real)cell.y + cellOffset) * PATHFIND_CELL_SIZE_F;
			nodePos.z = TheTerrainLogic->getLayerHeight(nodePos.x, nodePos.y, layer);
			theNewPath->appendNode(&nodePos, layer);
			if (theNewPath->getLastNode() && result.pathCanOptimize.size() > i)
				theNewPath->getLastNode()->setCanOptimize(result.pathCanOptimize[i]);
		}

		theNewPath->optimize(getObject(), m_locomotorSet.getValidSurfaces(), FALSE);
		if (!result.provisional && result.ultraAccurate && theNewPath->getLastNode())
			theNewPath->updateLastNode(&result.originalDestination);
	}
	...
	if (theNewPath)
	{
		...
		if (result.isFinalGoal && isDoingGroundMovement() && getPath())
			TheAI->pathfinder()->updateGoal(getObject(), getPath()->getLastNode()->getPosition(), getPath()->getLastNode()->getLayer());
	}
	else
	{
		if (!result.provisional)
...
void AIUpdateInterface::requestPath( Coord3D *destination, Bool isFinalGoal )
{
	...
	const Bool repeatedNoPathToSameDestination =
		m_path == nullptr &&
		!m_waitingForPath &&
		m_pathTimestamp != 0 &&
		TheGameLogic->getFrame() - m_pathTimestamp < REPEATED_NO_PATH_RETRY_FRAMES &&
		m_isAttackPath == FALSE &&
		m_isApproachPath == FALSE &&
		m_isSafePath == FALSE &&
		m_requestedVictimID == INVALID_ID &&
		m_isFinalGoal == isFinalGoal &&
		isSamePathTarget(getObject()->getPosition(), &m_requestedDestination, destination);
	...
	m_waitingForPath = TRUE;
	if (repeatedNoPathToSameDestination) {
		const Player *owner = getObject()->getControllingPlayer();
		Int cellX = REAL_TO_INT(getObject()->getPosition()->x / PATHFIND_CELL_SIZE);
		Int cellY = REAL_TO_INT(getObject()->getPosition()->y / PATHFIND_CELL_SIZE);
		PathfindCell *cell = TheAI->pathfinder()->getCell(getObject()->getLayer(), cellX, cellY);
		AsciiString botOrderContext;
		formatDebugBotOrderContext(botOrderContext);
		DEBUG_LOG(("%d PathRequest delay-repeat obj=%d tmpl='%s' player=%d cmd=%d state=%u pos=(%.3f,%.3f) dest=(%.3f,%.3f) blockedFrames=%u waiting=%d pathNodes=%d cell=(%d,%d,type=%s,pinched=%d) %s",
			TheGameLogic->getFrame(),
			getObject()->getID(),
			getObject()->getTemplate()->getName().str(),
			owner ? owner->getPlayerIndex() : -1,
			getLastCommandSource(),
			m_stateMachine ? m_stateMachine->getCurrentStateID() : INVALID_STATE_ID,
			getObject()->getPosition()->x, getObject()->getPosition()->y,
			destination->x, destination->y,
			m_blockedFrames,
			m_waitingForPath ? 1 : 0,
			debugCountPathNodes(m_path),
			cellX, cellY,
			debugPathCellTypeName(cell),
			cell ? (cell->getPinched() ? 1 : 0) : 0,
			botOrderContext.str()));
		setQueueForPathTime(REPEATED_NO_PATH_RETRY_FRAMES);
		return;
	}
	if (m_pathTimestamp > TheGameLogic->getFrame()-3) {
		...
		AsciiString botOrderContext;
		formatDebugBotOrderContext(botOrderContext);
		DEBUG_LOG(("%d PathRequest delay-rapid obj=%d tmpl='%s' player=%d cmd=%d state=%u age=%u pos=(%.3f,%.3f) dest=(%.3f,%.3f) blockedFrames=%u stuck=%d pathNodes=%d cell=(%d,%d,type=%s,pinched=%d) %s",
			TheGameLogic->getFrame(),
			getObject()->getID(),
			getObject()->getTemplate()->getName().str(),
			owner ? owner->getPlayerIndex() : -1,
			getLastCommandSource(),
			m_stateMachine ? m_stateMachine->getCurrentStateID() : INVALID_STATE_ID,
			TheGameLogic->getFrame() - m_pathTimestamp,
			getObject()->getPosition()->x, getObject()->getPosition()->y,
			destination->x, destination->y,
			m_blockedFrames,
			m_isBlockedAndStuck ? 1 : 0,
			debugCountPathNodes(m_path),
			cellX, cellY,
			debugPathCellTypeName(cell),
			cell ? (cell->getPinched() ? 1 : 0) : 0,
...
void AIUpdateInterface::requestAttackPath( ObjectID victimID, const Coord3D* victimPos )
{
	...
	const Bool repeatedNoPathToSameAttack =
		m_path == nullptr &&
		!m_waitingForPath &&
		m_pathTimestamp != 0 &&
		TheGameLogic->getFrame() - m_pathTimestamp < REPEATED_NO_PATH_RETRY_FRAMES &&
		...
		isSamePathTarget(getObject()->getPosition(), &m_requestedDestination, victimPos);
	...
	if (repeatedNoPathToSameAttack) {
		...
		AsciiString botOrderContext;
		formatDebugBotOrderContext(botOrderContext);
		DEBUG_LOG(("%d AttackPathRequest delay-repeat obj=%d tmpl='%s' player=%d cmd=%d state=%u victim=%u pos=(%.3f,%.3f) dest=(%.3f,%.3f) blockedFrames=%u waiting=%d pathNodes=%d cell=(%d,%d,type=%s,pinched=%d) %s",
			TheGameLogic->getFrame(),
			getObject()->getID(),
			getObject()->getTemplate()->getName().str(),
			owner ? owner->getPlayerIndex() : -1,
	...
	if (m_pathTimestamp > TheGameLogic->getFrame()-3) {
		...
		AsciiString botOrderContext;
		formatDebugBotOrderContext(botOrderContext);
		DEBUG_LOG(("%d AttackPathRequest delay-rapid obj=%d tmpl='%s' player=%d cmd=%d state=%u victim=%u age=%u pos=(%.3f,%.3f) dest=(%.3f,%.3f) blockedFrames=%u stuck=%d pathNodes=%d cell=(%d,%d,type=%s,pinched=%d) %s",
			TheGameLogic->getFrame(),
			getObject()->getID(),
			getObject()->getTemplate()->getName().str(),
			owner ? owner->getPlayerIndex() : -1,
...
void AIUpdateInterface::requestApproachPath( Coord3D *destination )
{
	...
	const Bool repeatedNoPathToSameApproach =
		m_path == nullptr &&
		!m_waitingForPath &&
		m_pathTimestamp != 0 &&
		TheGameLogic->getFrame() - m_pathTimestamp < REPEATED_NO_PATH_RETRY_FRAMES &&
	...
	m_isSafePath = FALSE;
	m_waitingForPath = TRUE;
	if (repeatedNoPathToSameApproach) {
		setQueueForPathTime(REPEATED_NO_PATH_RETRY_FRAMES);
		return;
	}
	if (m_pathTimestamp > TheGameLogic->getFrame()-3) {
...
Bool AIUpdateInterface::computePath( PathfindServicesInterface *pathServices, Coord3D *destination )
{
	...
	if (theNewPath==nullptr && m_path==nullptr) {
		...
		Int cellY = REAL_TO_INT(getObject()->getPosition()->y / PATHFIND_CELL_SIZE);
		PathfindCell *cell = TheAI->pathfinder()->getCell(getObject()->getLayer(), cellX, cellY);
		DEBUG_LOG(("%d ComputePath fallback-closest obj=%d tmpl='%s' player=%d state=%u pos=(%.3f,%.3f) dest=(%.3f,%.3f) final=%d blockedFrames=%u stuck=%d ignoreObstacle=%u cell=(%d,%d,type=%s,pinched=%d)",
			TheGameLogic->getFrame(),
			getObject()->getID(),
			getObject()->getTemplate()->getName().str(),
			owner ? owner->getPlayerIndex() : -1,
	...
	if (theNewPath) {
		...
		// Keep using the old path.
 		if (m_path && m_isBlockedAndStuck) {
			...
			Int cellY = REAL_TO_INT(getObject()->getPosition()->y / PATHFIND_CELL_SIZE);
			PathfindCell *cell = TheAI->pathfinder()->getCell(getObject()->getLayer(), cellX, cellY);
			DEBUG_LOG(("%d ComputePath stuck-reset obj=%d tmpl='%s' player=%d state=%u pos=(%.3f,%.3f) dest=(%.3f,%.3f) blockedFrames=%u pathNodes=%d cell=(%d,%d,type=%s,pinched=%d)",
				TheGameLogic->getFrame(),
				getObject()->getID(),
				getObject()->getTemplate()->getName().str(),
				owner ? owner->getPlayerIndex() : -1,
...
//-------------------------------------------------------------------------------------------------
void AIUpdateInterface::setDebugBotOrderContext(const char *reason, const char *verb, const Coord3D *targetPos, Int relatedPlayerIndex)
{
	++m_debugBotOrderSequence;
	m_debugBotOrderReason = (reason != nullptr && *reason != '\0') ? reason : nullptr;
	m_debugBotOrderVerb = (verb != nullptr && *verb != '\0') ? verb : nullptr;
	m_debugBotOrderFrame = TheGameLogic ? TheGameLogic->getFrame() : 0;
	if (getObject() != nullptr && getObject()->getPosition() != nullptr)
	{
		m_debugBotOrderIssuePos = *getObject()->getPosition();
	}
	else
	{
		m_debugBotOrderIssuePos.zero();
	}
	if (targetPos != nullptr)
	{
		m_debugBotOrderTargetPos = *targetPos;
	}
	else
	{
		m_debugBotOrderTargetPos.zero();
	}
	m_debugBotOrderRelatedPlayerIndex = relatedPlayerIndex;
}

//-------------------------------------------------------------------------------------------------
void AIUpdateInterface::clearDebugBotOrderContext()
{
	m_debugBotOrderReason = nullptr;
	m_debugBotOrderVerb = nullptr;
	m_debugBotOrderSequence = 0;
	m_debugBotOrderFrame = 0;
	m_debugBotOrderIssuePos.zero();
	m_debugBotOrderTargetPos.zero();
	m_debugBotOrderRelatedPlayerIndex = -1;
}

//-------------------------------------------------------------------------------------------------
void AIUpdateInterface::formatDebugBotOrderContext(AsciiString &summary) const
{
	if (m_debugBotOrderReason == nullptr || *m_debugBotOrderReason == '\0')
	{
		summary.format("botOrder=none");
		return;
	}

	const UnsignedInt curFrame = TheGameLogic ? TheGameLogic->getFrame() : m_debugBotOrderFrame;
	const UnsignedInt age = curFrame >= m_debugBotOrderFrame ? (curFrame - m_debugBotOrderFrame) : 0;
	const char *verb = (m_debugBotOrderVerb != nullptr && *m_debugBotOrderVerb != '\0') ? m_debugBotOrderVerb : "-";
	summary.format("botOrder={seq=%u age=%u reason=%s verb=%s issue=(%.0f,%.0f) target=(%.0f,%.0f)",
		m_debugBotOrderSequence,
		age,
		m_debugBotOrderReason,
		verb,
		m_debugBotOrderIssuePos.x,
		m_debugBotOrderIssuePos.y,
		m_debugBotOrderTargetPos.x,
		m_debugBotOrderTargetPos.y);
	if (m_debugBotOrderRelatedPlayerIndex >= 0)
	{
		AsciiString playerInfo;
		playerInfo.format(" player=%d", m_debugBotOrderRelatedPlayerIndex);
		summary.concat(playerInfo);
	}
	summary.concat("}");
}

...
StateReturnType AIInternalMoveToState::update()
{
	...
	if (m_waitingForPath)
	{
		...
		if (thePath==nullptr)
		{
			...
			if (loco != nullptr)
			{
				const Real closeEnoughDist = max(loco->getCloseEnoughDist(), PATHFIND_CELL_SIZE_F);
				const Real dx = obj->getPosition()->x - m_goalPosition.x;
				const Real dy = obj->getPosition()->y - m_goalPosition.y;
				if (dx * dx + dy * dy <= closeEnoughDist * closeEnoughDist)
				{
	...
	if (blocked)
	{
		const Bool logBlockedRepath = m_blockedRepathTimestamp == 0 ||
		...
		if (logBlockedRepath)
		{
			...
			const PathNode *lastNode = thePath ? thePath->getLastNode() : nullptr;
			AsciiString botOrderContext;
			ai->formatDebugBotOrderContext(botOrderContext);
			DEBUG_LOG(("%d MoveState blocked-repath obj=%d tmpl='%s' player=%d state=%u pos=(%.3f,%.3f) goal=(%.3f,%.3f) pathGoal=(%.3f,%.3f) blockedFrames=%u waiting=%d pathNodes=%d cell=(%d,%d,type=%s,pinched=%d,layer=%d) lastNode=(%.3f,%.3f) %s",
				TheGameLogic->getFrame(),
				obj->getID(),
				obj->getTemplate()->getName().str(),
				owner ? owner->getPlayerIndex() : -1,
				getMachine()->getCurrentStateID(),
				obj->getPosition()->x, obj->getPosition()->y,
				m_goalPosition.x, m_goalPosition.y,
				m_pathGoalPosition.x, m_pathGoalPosition.y,
				ai->getNumFramesBlocked(),
				ai->isWaitingForPath() ? 1 : 0,
				debugCountPathNodes(thePath),
				cellX, cellY,
				debugPathCellTypeName(cell),
				cell ? (cell->getPinched() ? 1 : 0) : 0,
				obj->getLayer(),
				lastNode ? lastNode->getPosition()->x : obj->getPosition()->x,
				lastNode ? lastNode->getPosition()->y : obj->getPosition()->y,
				botOrderContext.str()));
		}
	}
...
StateReturnType AIAttackMoveToState::update()
{
	...
	if (ret != STATE_CONTINUE) {
		...
		const Coord3D &retryGoalPosition = ai->getPath() ? m_pathGoalPosition : m_goalPosition;
		Real distSqr = sqr(owner->getPosition()->x - retryGoalPosition.x) + sqr(owner->getPosition()->y-retryGoalPosition.y);
		if (distSqr < sqr(ATTACK_CLOSE_ENOUGH_CELLS*PATHFIND_CELL_SIZE_F)) {
			return ret;
		}
		Int cellX = REAL_TO_INT(owner->getPosition()->x / PATHFIND_CELL_SIZE);
		Int cellY = REAL_TO_INT(owner->getPosition()->y / PATHFIND_CELL_SIZE);
		PathfindCell *cell = TheAI->pathfinder()->getCell(owner->getLayer(), cellX, cellY);
		const Player *controller = owner->getControllingPlayer();
		const PathNode *lastNode = ai->getPath() ? ai->getPath()->getLastNode() : nullptr;
		AsciiString botOrderContext;
		ai->formatDebugBotOrderContext(botOrderContext);
		DEBUG_LOG(("%d AIAttackMove retry obj=%d tmpl='%s' player=%d cmd=%d pos=(%.3f,%.3f) goal=(%.3f,%.3f) pathGoal=(%.3f,%.3f) lastNode=(%.3f,%.3f) dist=%.3f blockedFrames=%u waitUntil=%u retriesLeft=%d pathNodes=%d cell=(%d,%d,type=%s,pinched=%d,layer=%d) %s",
			TheGameLogic->getFrame(),
			owner->getID(),
			owner->getTemplate()->getName().str(),
			controller ? controller->getPlayerIndex() : -1,
			m_commandSrc,
			owner->getPosition()->x, owner->getPosition()->y,
			m_goalPosition.x, m_goalPosition.y,
			retryGoalPosition.x, retryGoalPosition.y,
			lastNode ? lastNode->getPosition()->x : owner->getPosition()->x,
			lastNode ? lastNode->getPosition()->y : owner->getPosition()->y,
			sqrt(distSqr),
			ai->getNumFramesBlocked(),
			m_frameToSleepUntil,
			m_retryCount,
			debugCountPathNodes(ai->getPath()),
			cellX, cellY,
			debugPathCellTypeName(cell),
			cell ? (cell->getPinched() ? 1 : 0) : 0,
			owner->getLayer(),

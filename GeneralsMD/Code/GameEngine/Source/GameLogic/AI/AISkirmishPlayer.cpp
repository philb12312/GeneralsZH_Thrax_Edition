...
// Fine-grained perf hooks provided by GameEngine.cpp.
...
static void BotAILog(const char *fmt, ...)
{
	if (!g_botAILogOpened)
	{
		...
		if (g_botAILogFile)
		{
			...
			fprintf(g_botAILogFile, "=== Build: Zero Hour (Release) ===\n");
...
namespace
{
	...
	const UnsignedInt TEAM_COMMAND_LOCK_FRAMES = 32 * LOGICFRAMES_PER_SECOND;
	const UnsignedInt GROUP_ATTACK_BREAKOUT_FRAMES = 6 * LOGICFRAMES_PER_SECOND;
	const Real GROUP_ATTACK_BREAKOUT_DISTANCE_SCALE = 2.2f;
	const Real GROUP_ATTACK_BREAKOUT_DISTANCE_PADDING = 450.0f;
	const Real GROUP_ATTACK_RELEASE_DISTANCE_SCALE = 1.55f;
	const Real GROUP_ATTACK_RELEASE_DISTANCE_PADDING = 250.0f;
	const Real GROUP_ATTACK_BREAKOUT_POINT_CLOSE_ENOUGH = 220.0f;
	// Cooldown after a bot receives a new plan (human command or own choice) before it
	// will re-plan. Previously 300s which effectively prevented any coordinated attack
	// from self-initiating during a match. 60s keeps us from thrashing but lets bots
	// team up regularly.
	const UnsignedInt PLAN_CHANGE_COOLDOWN_FRAMES = 60 * LOGICFRAMES_PER_SECOND;
	...
	/**
	 * Return a human-readable color name for a player based on their minimap color.
	 * Prefers the game's own color definitions from MultiplayerSettings (e.g. the
	 * tooltip name defined in Multiplayer.ini) so our names match the lobby UI
	 * instead of guessing from raw RGB. Falls back to a crude RGB classifier
	 * only if no matching color definition can be found (e.g. custom colors).
	 */
	AsciiString getPlayerColorName(const Player *player)
	{
		if (player == nullptr)
		{
			return AsciiString("enemy");
		}

		Color c = player->getPlayerColor();

		// Try to look up the real color name from the game's multiplayer settings.
		if (TheMultiplayerSettings != nullptr)
		{
			const Int numColors = TheMultiplayerSettings->getNumColors();
			MultiplayerColorDefinition *bestExact = nullptr;
			MultiplayerColorDefinition *bestNear = nullptr;
			Int bestNearScore = 0x7fffffff;
			UnsignedByte pr = 0, pg = 0, pb = 0, pa = 0;
			GameGetColorComponents(c, &pr, &pg, &pb, &pa);
			for (Int i = 0; i < numColors; ++i)
			{
				MultiplayerColorDefinition *def = TheMultiplayerSettings->getColor(i);
				if (def == nullptr) continue;
				// Exact match on packed Color value (ignore alpha differences).
				if ((def->getColor() & 0x00FFFFFF) == (c & 0x00FFFFFF))
				{
					bestExact = def;
					break;
				}
				// Nearest-color fallback (rare: if game re-packs alpha differently).
				UnsignedByte dr = 0, dg = 0, db = 0, da = 0;
				GameGetColorComponents(def->getColor(), &dr, &dg, &db, &da);
				const Int diffR = (Int)dr - (Int)pr;
				const Int diffG = (Int)dg - (Int)pg;
				const Int diffB = (Int)db - (Int)pb;
				const Int score = diffR*diffR + diffG*diffG + diffB*diffB;
			...
			MultiplayerColorDefinition *picked = bestExact ? bestExact : bestNear;
			if (picked != nullptr)
			{
				AsciiString tip = picked->getTooltipName();
				// Tooltip keys look like "Color:Red" - strip the label prefix so we
				// return just the english base name that matches player chat keywords.
				const char *raw = tip.str();
				const char *colon = raw ? strrchr(raw, ':') : nullptr;
				AsciiString name;
				if (colon && *(colon + 1))
					name = AsciiString(colon + 1);
				else if (raw && *raw)
					name = tip;
				if (!name.isEmpty())
					return name;
		...
		// Fallback: crude RGB classifier (only used if settings unavailable).
		UnsignedByte r = 0, g = 0, b = 0, a = 0;
		GameGetColorComponents(c, &r, &g, &b, &a);
		if (r > 180 && g < 80 && b < 80)  return AsciiString("Red");
		if (b > 160 && r < 80 && g < 80)  return AsciiString("Blue");
		if (g > 160 && r < 80 && b < 80)  return AsciiString("Green");
		if (r > 180 && g > 100 && g < 180 && b < 60) return AsciiString("Orange");
		if (r > 180 && g > 180 && b < 80) return AsciiString("Yellow");
		if (g > 160 && b > 160 && r < 80) return AsciiString("Teal");
		if (r > 100 && b > 100 && g < 80) return AsciiString("Purple");
		if (r > 180 && b > 100 && g < 100) return AsciiString("Pink");
		if (r > 200 && g > 200 && b > 200) return AsciiString("White");
		if (r < 50 && g < 50 && b < 50) return AsciiString("Black");
	...
	/**
	 * User-facing label for a player. If the target shares a color name with the
	 * local (human) player, we disambiguate with the player's actual name so the
	 * human doesn't mistake bot chat like "Attacking Red" for being attacked
	 * themselves. Used only for on-screen chat and ping text, never for logs.
	 */
	AsciiString getPlayerDisplayLabel(const Player *target)
	{
		if (target == nullptr) return AsciiString("enemy");
		AsciiString colorName = getPlayerColorName(target);
		const Player *localPlayer = ThePlayerList ? ThePlayerList->getLocalPlayer() : nullptr;
		if (localPlayer == nullptr || localPlayer == target)
			return colorName;
		AsciiString localColor = getPlayerColorName(localPlayer);
		if (colorName.compareNoCase(localColor.str()) != 0)
			return colorName;
		// Same color as local player -> disambiguate with player display name.
		UnicodeString displayNameU = const_cast<Player *>(target)->getPlayerDisplayName();
		AsciiString displayNameA;
		displayNameA.translate(displayNameU);
		AsciiString result;
		if (!displayNameA.isEmpty())
			result.format("%s player %s", colorName.str(), displayNameA.str());
		else
			result.format("the other %s player", colorName.str());
		return result;
	}
	...
	Bool tryGetDirectionalTargetPosition(const AsciiString &command, Coord3D *targetPos)
	{
		...
		const Bool hasTop = containsText(command, "top") || containsText(command, "upper") || containsText(command, "north");
		const Bool hasBottom = containsText(command, "bottom") || containsText(command, "lower") || containsText(command, "south");
		...
		if (hasTop)
		{
			// In Generals map coordinates, larger Y is the visually upper/northern side
			// of the map (see adjustBuildList's 3x3 grid comment). Keep chat directions
			// aligned with the player's screen, not the raw low-Y/high-Y intuition.
	...
	{
		Bool hasTop    = containsText(command, "top")    || containsText(command, "upper") || containsText(command, "north");
		Bool hasBottom = containsText(command, "bottom") || containsText(command, "lower") || containsText(command, "south");
	...
	Bool shouldIgnoreCombatOrderObject(const Object *obj)
	{
		if (obj == nullptr)
		{
			return true;
	...
	void appendCombatOrderSample(AsciiString &samples, Int &sampleCount, const Object *obj)
	{
		...
		sample.format("%s#%d", templ ? templ->getName().str() : "Unknown", obj->getID());
		if (!samples.isEmpty())
		{
			samples.concat(", ");
		}
	...
	Real maxReal(Real a, Real b)
	{
		return a > b ? a : b;
	}

	Real distanceSquared2D(const Coord3D &a, const Coord3D &b)
	{
		return sqr(a.x - b.x) + sqr(a.y - b.y);
	}

	Bool hasNonZero2D(const Coord3D &pos)
	{
		return pos.x != 0.0f || pos.y != 0.0f;
	}

	Object *findRepresentativeGroundCombatUnit(Player *player, const Coord3D &referencePos)
	{
		if (player == nullptr || player->getPlayerTeams() == nullptr)
		{
			return nullptr;
		}

		Object *bestObject = nullptr;
		Real bestDistSq = HUGE_DIST * HUGE_DIST;
		for (Player::PlayerTeamList::const_iterator teamProtoIt = player->getPlayerTeams()->begin();
			teamProtoIt != player->getPlayerTeams()->end();
			++teamProtoIt)
		{
			for (DLINK_ITERATOR<Team> iter = (*teamProtoIt)->iterate_TeamInstanceList(); !iter.done(); iter.advance())
			{
				...
				for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList(); !objIter.done(); objIter.advance())
				{
					...
					if (shouldIgnoreCombatOrderObject(obj) || obj->isKindOf(KINDOF_AIRCRAFT))
					{
						continue;
					}

					if (obj->getAIUpdateInterface() == nullptr || obj->getPosition() == nullptr)
					{
						continue;
					}

					Real distSq = distanceSquared2D(*obj->getPosition(), referencePos);
					if (bestObject == nullptr || distSq < bestDistSq)
					{
						bestObject = obj;
						bestDistSq = distSq;
					}
	...
	Bool isUsableCoordinatedBreakoutPoint(const Object *sampleUnit, const Coord3D *candidatePos)
	{
		if (sampleUnit == nullptr || candidatePos == nullptr || TheAI == nullptr || TheAI->pathfinder() == nullptr || TheTerrainLogic == nullptr)
		{
			return false;
		}

		const AIUpdateInterface *sampleAI = sampleUnit->getAIUpdateInterface();
		const Coord3D *samplePos = sampleUnit->getPosition();
		if (sampleAI == nullptr || samplePos == nullptr)
		{
			return false;
		}

		Pathfinder *pathfinder = TheAI->pathfinder();
		const LocomotorSet &locomotorSet = sampleAI->getLocomotorSet();
		if (TheTerrainLogic->isCliffCell(candidatePos->x, candidatePos->y)
			|| TheTerrainLogic->isUnderwater(candidatePos->x, candidatePos->y, nullptr, nullptr))
		{
			return false;
		}

		PathfindLayerEnum candidateLayer = TheTerrainLogic->getLayerForDestination(candidatePos);
		PathfindCell *candidateCell = pathfinder->getCell(candidateLayer, candidatePos);
		if (candidateCell == nullptr)
		{
			return false;
		}

		const PathfindCell::CellType candidateType = candidateCell->getType();
		if (candidateType == PathfindCell::CELL_CLIFF
			|| candidateType == PathfindCell::CELL_WATER
			|| candidateType == PathfindCell::CELL_OBSTACLE
			|| candidateType == PathfindCell::CELL_BRIDGE_IMPASSABLE
			|| candidateType == PathfindCell::CELL_IMPASSABLE
			|| candidateCell->getPinched())
		{
			return false;
		}

		if (!pathfinder->validMovementPosition(false, candidateLayer, locomotorSet, candidatePos))
		{
			return false;
		}

		return pathfinder->clientSafeQuickDoesPathExist(locomotorSet, samplePos, candidatePos);
	}

	Bool chooseCoordinatedAttackBreakoutPoint(Player *player, const Coord3D &baseCenter, Real baseRadius, const Coord3D *targetPos, Coord3D *outBreakoutPoint)
	{
		if (player == nullptr || targetPos == nullptr || outBreakoutPoint == nullptr || TheAI == nullptr || TheAI->pathfinder() == nullptr || TheTerrainLogic == nullptr)
		{
			return false;
		}

		Object *sampleUnit = findRepresentativeGroundCombatUnit(player, baseCenter);
		if (sampleUnit == nullptr)
		{
			BotAILog("%s | GROUP: breakout point unavailable (no representative ground unit)", getBotLogPrefix(player).str());
			return false;
		}
		...
		if (sampleAI == nullptr || samplePos == nullptr)
		{
			BotAILog("%s | GROUP: breakout point unavailable (representative unit missing AI/position)", getBotLogPrefix(player).str());
			return false;
		}

		const Real breakoutDistance = maxReal(baseRadius * GROUP_ATTACK_BREAKOUT_DISTANCE_SCALE, baseRadius + GROUP_ATTACK_BREAKOUT_DISTANCE_PADDING);
		const Real breakoutDistanceSq = sqr(breakoutDistance);
		Bool found = false;

		Coord3D adjustedTarget = *targetPos;
		PathfindServicesInterface *pathServices = TheAI->pathfinder()->getPathfindServices();
		Path *path = pathServices ? pathServices->findClosestPath(sampleUnit, sampleAI->getLocomotorSet(), samplePos, &adjustedTarget, FALSE, 0.0f, FALSE) : nullptr;
		if (path != nullptr)
		{
			for (PathNode *node = path->getFirstNode(); node != nullptr; node = node->getNext())
			{
				...
				if (distanceSquared2D(*nodePos, baseCenter) < breakoutDistanceSq)
				{
					continue;
				}

				if (!isUsableCoordinatedBreakoutPoint(sampleUnit, nodePos))
				{
					continue;
				}

				*outBreakoutPoint = *nodePos;
				found = true;
				break;
			}
			deleteInstance(path);
		...
		{
			Coord2D dir;
			dir.x = targetPos->x - baseCenter.x;
			dir.y = targetPos->y - baseCenter.y;
			Real len = sqr(dir.x) + sqr(dir.y);
			if (len <= 1.0f)
			{
				BotAILog("%s | GROUP: breakout point unavailable (zero direction vector)", getBotLogPrefix(player).str());
				return false;
			}
			...
			Coord3D fallbackPoint = baseCenter;
			fallbackPoint.x += dir.x * breakoutDistance;
			fallbackPoint.y += dir.y * breakoutDistance;
			fallbackPoint.z = TheTerrainLogic->getGroundHeight(fallbackPoint.x, fallbackPoint.y);
			if (isUsableCoordinatedBreakoutPoint(sampleUnit, &fallbackPoint))
			{
				*outBreakoutPoint = fallbackPoint;
				found = true;
			}
		}
		...
		if (found)
		{
			BotAILog("%s | GROUP: breakout point chosen at (%.0f,%.0f) for target (%.0f,%.0f)",
				getBotLogPrefix(player).str(), outBreakoutPoint->x, outBreakoutPoint->y, targetPos->x, targetPos->y);
		}
		else
		{
			BotAILog("%s | GROUP: breakout point unavailable -> falling back to legacy launch", getBotLogPrefix(player).str());
		}
...
m_coordAttackBreakoutUntilFrame(0),
m_coordAttackPending(false),
m_coordAttackBreakoutPending(false),
...
m_frameNextCoordSkipLog(0),
m_coordAttackLastLoggedLeadIdx(-1),
m_humanCommandIsAttack(false),
m_humanCommandAttackEnemyIndex(PLAYER_INDEX_INVALID)

{
	m_frameLastBuildingBuilt = TheGameLogic->getFrame();
	p->setCanBuildUnits(true); // turn on ai production by default.
	m_coordAttackTarget.zero();
	m_coordBreakoutPoint.zero();
...
void AISkirmishPlayer::checkReadyTeams()
{
	...
	if (m_coordAttackPending && (m_coordRallyPoint.x != 0.0f || m_coordRallyPoint.y != 0.0f))
	{
		...
		for (Int i = 0; i < graduatedTeamCount; ++i)
			sent += sendCombatTeamToPosition(graduatedTeams[i], &m_coordRallyPoint, false,
				"graduated_group_rally", m_coordAttackEnemyPlayerIndex);
	...
	else if (m_coordAttackBreakoutPending && hasNonZero2D(m_coordBreakoutPoint))
	{
		Int sent = 0;
		for (Int i = 0; i < graduatedTeamCount; ++i)
			sent += sendCombatTeamToMovePosition(graduatedTeams[i], &m_coordBreakoutPoint,
				"graduated_group_breakout", m_coordAttackEnemyPlayerIndex);
		BotAILog("%s | checkReadyTeams: %d team(s) graduated during breakout -> %d units move (%.0f,%.0f)",
			getBotLogPrefix(m_player).str(), graduatedTeamCount, sent, m_coordBreakoutPoint.x, m_coordBreakoutPoint.y);
	}
	else if (!m_coordAttackPending && m_holdAttackUntilFrame > curFrame
		&& (m_coordRallyPoint.x != 0.0f || m_coordRallyPoint.y != 0.0f))
	{
		if (m_humanCommandIsAttack)
		{
			Int sent = 0;
			for (Int i = 0; i < graduatedTeamCount; ++i)
				sent += sendCombatTeamToPosition(graduatedTeams[i], &m_coordRallyPoint, false,
					"graduated_human_attack_hold", m_humanCommandAttackEnemyIndex);
			BotAILog("%s | checkReadyTeams: %d team(s) graduated during attack hold -> %d units attack-move (%.0f,%.0f)",
				getBotLogPrefix(m_player).str(), graduatedTeamCount, sent, m_coordRallyPoint.x, m_coordRallyPoint.y);
		}
		else
		{
			...
			for (Int i = 0; i < graduatedTeamCount; ++i)
				sent += sendCombatTeamToPosition(graduatedTeams[i], &m_coordRallyPoint, true,
					"graduated_human_guard_hold", PLAYER_INDEX_INVALID);
		...
		&& (m_requestedAssistTarget.x != 0.0f || m_requestedAssistTarget.y != 0.0f))
	{
		...
		for (Int i = 0; i < graduatedTeamCount; ++i)
			sent += sendCombatTeamToPosition(graduatedTeams[i], &m_requestedAssistTarget, true,
				"graduated_assist_guard", m_requestedAssistPlayerIndex);
...
{
	const double kMsPerTick = PerfGetFreqQuadPartAsMsMultiplier();
	LARGE_INTEGER _qpcA, _qpcB;
	#define ASP_PERF_BEGIN() QueryPerformanceCounter(&_qpcA)
	#define ASP_PERF_END(tag) do { QueryPerformanceCounter(&_qpcB); \
		if (kMsPerTick > 0.0) PerfRecordLogicSub((tag), (double)(_qpcB.QuadPart - _qpcA.QuadPart) * kMsPerTick); } while(0)

	ASP_PERF_BEGIN();
	AIPlayer::update();
	ASP_PERF_END("Lg_ASP_Base");
	...
	if (!m_coordAttackPending && m_holdAttackUntilFrame > 0 && curFrame >= m_holdAttackUntilFrame)
	{
		...
		m_humanCommandIsAttack = false;
		m_humanCommandAttackEnemyIndex = PLAYER_INDEX_INVALID;
		clearAllCombatTeamCommandLocks();
	}
	...
	if (curFrame >= m_frameNextMoveReissue)
	{
		...
		if (!m_coordAttackPending && m_holdAttackUntilFrame > curFrame && (m_coordRallyPoint.x != 0.0f || m_coordRallyPoint.y != 0.0f))
		{
			...
			if (m_humanCommandIsAttack)
			{
				sent = sendAllCombatTeamsToPosition(&m_coordRallyPoint, "human_attack_refresh", m_humanCommandAttackEnemyIndex);
				BotAILog("%s | attack-hold refresh -> %d units attack-move to (%.0f,%.0f)  holdUntil=%u",
					getBotLogPrefix(m_player).str(), sent, m_coordRallyPoint.x, m_coordRallyPoint.y, m_holdAttackUntilFrame);
			}
			else
			{
				sent = sendAllCombatTeamsToGuardPosition(&m_coordRallyPoint, "human_guard_refresh", PLAYER_INDEX_INVALID);
		...
		{
			m_frameNextMoveReissue = curFrame + REISSUE_INTERVAL;
			Int sent = sendAllCombatTeamsToGuardPosition(&m_requestedAssistTarget, "assist_guard_refresh", m_requestedAssistPlayerIndex);
	...
	// Run coordination FIRST so a group attack can be staged before assistAlliedPlayers
	// drains all idle teams toward a damaged ally every 5 seconds.
	ASP_PERF_BEGIN();
	coordinateGroupAttack();
	ASP_PERF_END("Lg_ASP_Coord");

	ASP_PERF_BEGIN();
	assistAlliedPlayers();
	ASP_PERF_END("Lg_ASP_Assist");

	#undef ASP_PERF_BEGIN
	#undef ASP_PERF_END
}
...
void AISkirmishPlayer::onChatMessage(const Player *sender, const UnicodeString &text, Int playerMask)
{
	...
	if (containsText(command, "status") || containsText(command, "report"))
	{
		...
		else if (m_coordAttackPending && m_coordAttackEnemyPlayerIndex != PLAYER_INDEX_INVALID)
		{
			...
			status.format("Attacking %s.", getPlayerDisplayLabel(enemy).str());
		}
		else if (m_requestedAssistUntilFrame > curFrame && m_requestedAssistPlayerIndex != PLAYER_INDEX_INVALID)
		{
			Player *ally = ThePlayerList->getNthPlayer(m_requestedAssistPlayerIndex);
			status.format("Supporting %s.", getPlayerDisplayLabel(ally).str());
		}
		else
		{
			Player *enemy = getAiEnemy();
			if (enemy != nullptr)
				status.format("Watching %s.", getPlayerDisplayLabel(enemy).str());
			else
				status = "Building up.";
		}
	...
	if (containsText(command, "ping") || containsText(command, "where are you") || containsText(command, "location"))
	{
		...
		{
			Player *enemy = ThePlayerList->getNthPlayer(m_coordAttackEnemyPlayerIndex);
			pingText.format("Marking attack on %s.", getPlayerDisplayLabel(enemy).str());
		}
		else if (m_requestedAssistUntilFrame > curFrame && m_requestedAssistPlayerIndex != PLAYER_INDEX_INVALID)
		{
			Player *ally = ThePlayerList->getNthPlayer(m_requestedAssistPlayerIndex);
			pingText.format("Marking support for %s.", getPlayerDisplayLabel(ally).str());
		}
		else
		{
		...
		|| containsText(command, "backup me"))
	{
		...
		Int unitsSent = sendAllCombatTeamsToPosition(&assistPos, "chat_assist_attackmove", sender->getPlayerIndex());
	...
	if (wantsMoveOrder)
	{
		...
		Int moveRelatedPlayerIndex = PLAYER_INDEX_INVALID;
		...
		if (haveMovePos)
		{
			...
			Region3D mapBounds;
			TheTerrainLogic->getMaximumPathfindExtent(&mapBounds);
			BotAILog("%s |   directional parse OK -> label='%s' pos=(%.0f,%.0f) bounds=lo(%.0f,%.0f) size=(%.0f,%.0f) [top/upper=highY bottom/lower=lowY]",
				getBotLogPrefix(m_player).str(), moveLabel.str(), movePos.x, movePos.y,
				mapBounds.lo.x, mapBounds.lo.y, mapBounds.width(), mapBounds.height());
		}
		else if (containsText(command, "come here") || containsText(command, "meet") || containsText(command, "rally"))
		{
			...
			haveMovePos = true;
			moveRelatedPlayerIndex = sender->getPlayerIndex();
		...
		else
		{
			...
			if (colorTarget != nullptr)
			{
				...
				movePos.y = bounds.lo.y + bounds.height() / 2;
				movePos.z = TheTerrainLogic->getGroundHeight(movePos.x, movePos.y);
				moveLabel = getPlayerDisplayLabel(colorTarget);
				isPlayerTarget = true;
				moveRelatedPlayerIndex = colorTarget->getPlayerIndex();
				haveMovePos = true;
		...
		if (!haveMovePos)
		{
			...
			if (isDirectlyTargeted || isFirstAlliedBotForPlayer(sender))
				sendBotChat("Tell me where: center/top/bottom/lower/left/right or a player color.", false, true);
			return;
		}
		...
		m_frameNextMoveReissue = curFrame + 30 * LOGICFRAMES_PER_SECOND; // backup guard refresh in 30s
		m_humanCommandIsAttack = false; // move -> guard at destination
		m_humanCommandAttackEnemyIndex = PLAYER_INDEX_INVALID;

		Int unitsSent = sendAllCombatTeamsToPosition(&movePos, "chat_move_attackmove", moveRelatedPlayerIndex);
	...
	if (wantsAttackOrder)
	{
		...
		BotAILog("%s |   target='%s'  pos=(%.0f,%.0f)  isFirstBot=%d  idleTeams=%d",
			getBotLogPrefix(m_player).str(), getPlayerColorName(enemy).str(),
			targetPos.x, targetPos.y, (int)isFirstAlliedBotForPlayer(sender), (int)hasIdleCombatTeams());

		// Cancel any staged coordination and lock in this human command.
		cancelCoordinatedAttack();
		m_requestedAssistUntilFrame = 0;
		m_requestedAssistPlayerIndex = PLAYER_INDEX_INVALID;
		m_requestedAssistTarget.zero();
		...
		// Persist the attack order: re-issue every 30s and redirect newly-built teams.
		m_holdAttackUntilFrame = curFrame + DIRECT_MOVE_HOLD_FRAMES;
		m_coordRallyPoint = targetPos;
		m_humanCommandIsAttack = true;
		m_humanCommandAttackEnemyIndex = enemy->getPlayerIndex();
		m_frameNextMoveReissue = curFrame + 30 * LOGICFRAMES_PER_SECOND;

		// Launch units directly - no staging delay for explicit human commands.
		Int unitsSent = sendAllCombatTeamsToPosition(&targetPos, "chat_attack_order", enemy->getPlayerIndex());
		BotAILog("%s |   sendAllCombatTeamsToPosition -> %d units dispatched to (%.0f,%.0f)  holdUntil=%u",
			getBotLogPrefix(m_player).str(), unitsSent, targetPos.x, targetPos.y, m_holdAttackUntilFrame);

		// Every targeted bot announces.
		{
			AsciiString msg;
			if (unitsSent > 0)
				msg.format("Attacking %s.", getPlayerDisplayLabel(enemy).str());
			else
				msg.format("No units yet, but I'll hit %s once I have forces.", getPlayerDisplayLabel(enemy).str());
	...
	if (containsText(command, "help") || containsText(command, "commands") || containsText(command, "what can you do"))
	{
		if (isDirectlyTargeted || isFirstAlliedBotForPlayer(sender))
		{
			sendBotChat("Commands: move [dir/color], attack [dir/color], assist me, ping, status, stop", false, true);
		}
		return;
	}

	// --- Unknown command ---
	// If a bot was directly addressed by color, it must always respond (malformed-command feedback).
	// Otherwise only the first allied bot answers to avoid triple-replies on broadcast noise.
	if (isDirectlyTargeted)
	{
		AsciiString reply;
		reply.format("I don't understand '%s'. Try: move <dir/color>, attack <dir/color>, assist me, ping, status, stop.", command.str());
		sendBotChat(reply, false, true);
	}
	else if (isFirstAlliedBotForPlayer(sender))
	{
		sendBotChat("Commands: move [dir/color], attack [dir/color], assist me, ping, status, stop", false, true);
	}
}
...
void AISkirmishPlayer::queueCoordinatedAttack(const Coord3D *targetPos, const Player *enemy, UnsignedInt waitStartFrame)
{
	...
	const UnsignedInt curFrame = TheGameLogic ? TheGameLogic->getFrame() : waitStartFrame;
	if (!wasPending)
	{
		// Joining/staging a group attack is a real plan change. Without this, bots
		// force-launched by an ally could immediately self-initiate a fresh staging
		// order and pull armies back toward their rally point.
		m_frameLastPlanChange = curFrame;
		m_frameNextCoordSkipLog = 0;
	}
	...
	if (chooseCoordinatedAttackBreakoutPoint(m_player, m_baseCenter, m_baseRadius, targetPos, &m_coordBreakoutPoint))
	{
		m_coordRallyPoint = m_coordBreakoutPoint;
	}

	if (!wasPending)
	{
		Int sent = sendIdleCombatTeamsToPosition(&m_coordRallyPoint, "group_stage_idle_rally", enemy->getPlayerIndex());
		BotAILog("%s | GROUP: staging order (idle teams only) -> %d units rally to (%.0f,%.0f)",
		...
		AsciiString msg;
		msg.format("Grouping to attack %s.", getPlayerDisplayLabel(enemy).str());
		sendBotChat(msg, true, false);
	}
	else
	{
		// Keep newly-idle teams moving toward staging without re-pathing every unit.
		rallyTeamsToStagingPoint("group_stage_rally_refresh", m_coordAttackEnemyPlayerIndex);
	}
}
...
void AISkirmishPlayer::sendBotChat(const AsciiString &message, Bool ping, Bool force)
{
	...
	*nextAllowedFrame = curFrame + cooldown;
	...
	// Use bIsChatMsg=true so the message uses messageTimeoutChat (full chat duration).
	// Without this, isChat=false → messageTimeoutStandard rounds to 0 frames → message invisible.
	TheInGameUI->messageColor(true, &rgb, UnicodeString(L"%ls"), finalLine.str());
	...
	if (ping && TheRadar != nullptr)
	{
		...
		else if (m_humanCommandIsAttack && m_holdAttackUntilFrame > curFrame
			&& (m_coordRallyPoint.x != 0.0f || m_coordRallyPoint.y != 0.0f))
		{
			pingPos = m_coordRallyPoint;
			havePingPos = true;
			BotAILog("%s |   ping -> HUMAN ATTACK TARGET  (%.0f,%.0f)", getBotLogPrefix(m_player).str(), pingPos.x, pingPos.y);
		}
		else if (m_requestedAssistUntilFrame > curFrame && m_requestedAssistPlayerIndex != PLAYER_INDEX_INVALID)
		{
		...
		if (havePingPos)
		{
			// Offset each bot's ping slightly so multiple bots responding to the same command
			// (e.g. "move yellow" with two bot allies) don't all draw on top of each other.
			// Spread in world-space; fits comfortably inside the minimap triangle.
			const Int pIdx = (m_player ? m_player->getPlayerIndex() : 0);
			static const Real kPingSpread = 120.0f;
			static const Real kOffsetX[8] = { 0.0f,  kPingSpread, -kPingSpread, 0.0f,  kPingSpread, -kPingSpread, 0.6f * kPingSpread, -0.6f * kPingSpread };
			static const Real kOffsetY[8] = { 0.0f,  0.0f,        0.0f,         kPingSpread, kPingSpread,  kPingSpread, -kPingSpread, -kPingSpread };
			const Int slot = (pIdx >= 0 && pIdx < 8) ? pIdx : 0;
			pingPos.x += kOffsetX[slot];
			pingPos.y += kOffsetY[slot];
			// Diagnostic dump: raw player color + resolved color name + packed RGB of
			// each ingame lobby-color slot that matches (or is closest). This lets us
			// verify whether the color the user sees on the minimap (which is drawn
			// directly from m_player->getPlayerColor()) actually matches the color
			// name we resolved for chat/log (via TheMultiplayerSettings). If the two
			// ever disagree, we know the lobby->player color pipeline is broken.
			const UnsignedInt rawColor = (UnsignedInt)m_player->getPlayerColor();
			UnsignedByte cr = 0, cg = 0, cb = 0, ca = 0;
			GameGetColorComponents((Color)rawColor, &cr, &cg, &cb, &ca);
			BotAILog("%s |   ping FIRED at (%.0f,%.0f)  (slot=%d offset=(%.0f,%.0f))  rawColor=0x%08X rgba=(%u,%u,%u,%u) resolvedName='%s'",
				getBotLogPrefix(m_player).str(), pingPos.x, pingPos.y, slot, kOffsetX[slot], kOffsetY[slot],
				rawColor, cr, cg, cb, ca, getPlayerColorName(m_player).str());
			// Use createPlayerEvent so the ping draws in THIS bot's player color
			// (the generic createEvent uses a fixed yellow/INFORMATION palette entry).
			TheRadar->createPlayerEvent(m_player, &pingPos, RADAR_EVENT_INFORMATION, 12.0f);
		}
	}
...
Bool AISkirmishPlayer::hasCombatTeams() const
{
		...
		++teamProtoIt)
	{
		for (DLINK_ITERATOR<Team> iter = (*teamProtoIt)->iterate_TeamInstanceList(); !iter.done(); iter.advance())
		{
			...
			{
				Object *obj = objIter.cur();
				if (shouldIgnoreCombatOrderObject(obj))
				{
					continue;
				}
...
Int AISkirmishPlayer::sendCombatTeamToPosition(Team *team, const Coord3D *targetPos, Bool guardMode, const char *debugReason, Int relatedPlayerIndex)
{
	...
	const UnsignedInt lockUntil = curFrame + TEAM_COMMAND_LOCK_FRAMES;
	...
	for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList(); !objIter.done(); objIter.advance())
	{
		...
		if (ai != nullptr)
		{
			if (!issuedOrder && lockUntil > team->getCommandLockUntilFrame())
			{
...
//----------------------------------------------------------------------------------------------------------
/**
 * Send one combat team to a position with a pure move order.
 */
Int AISkirmishPlayer::sendCombatTeamToMovePosition(Team *team, const Coord3D *targetPos, const char *debugReason, Int relatedPlayerIndex)
{
	if (team == nullptr || targetPos == nullptr || !team->hasAnyUnits())
	{
		return 0;
	...
	const UnsignedInt curFrame = TheGameLogic ? TheGameLogic->getFrame() : 0;
	const UnsignedInt lockUntil = curFrame + TEAM_COMMAND_LOCK_FRAMES;
	const char *orderReason = resolveCombatOrderReason(debugReason, "combat_team_move");
	...
	for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList(); !objIter.done(); objIter.advance())
	{
		...
		if (ai != nullptr)
		{
			...
			issuedOrder = true;
			noteCombatOrderContext(ai, targetPos, orderReason, "move", relatedPlayerIndex);
			ai->aiMoveToPosition(targetPos, CMD_FROM_AI);
			appendIssuedCombatOrderSample(issuedSamples, issuedSampleCount, obj);
			++unitsSent;
		}
	}
	...
	if (unitsSent > 0 && !issuedSamples.isEmpty())
	{
		BotAILog("%s | sendCombatTeamToMovePosition (%.0f,%.0f): sample=%s",
			getBotLogPrefix(m_player).str(), targetPos->x, targetPos->y, issuedSamples.str());
	}
...
//----------------------------------------------------------------------------------------------------------
/**
 * Send every available idle combat team to guard a target position.
 * Used for automatic ally defense so units stay committed instead of repeatedly
 * becoming idle and being re-dispatched every 5 seconds.
 */
Int AISkirmishPlayer::sendIdleCombatTeamsToGuardPosition(const Coord3D *targetPos, const char *debugReason, Int relatedPlayerIndex)
{
	if (targetPos == nullptr)
	{
		return 0;
	...
	const UnsignedInt curFrame = TheGameLogic ? TheGameLogic->getFrame() : 0;
	const UnsignedInt lockUntil = curFrame + TEAM_COMMAND_LOCK_FRAMES;
	const char *orderReason = resolveCombatOrderReason(debugReason, "idle_guard");
	...
	Int unitsSkippedBusy = 0;
	Int unitsSkippedNoAI = 0;
	AsciiString noAIExamples;
	Int noAISampleCount = 0;
	AsciiString issuedSamples;
	Int issuedSampleCount = 0;
	for (Player::PlayerTeamList::const_iterator teamProtoIt = m_player->getPlayerTeams()->begin();
		teamProtoIt != m_player->getPlayerTeams()->end();
		++teamProtoIt)
	{
		for (DLINK_ITERATOR<Team> iter = (*teamProtoIt)->iterate_TeamInstanceList(); !iter.done(); iter.advance())
		{
			...
			for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList(); !objIter.done(); objIter.advance())
			{
				...
				{
					++unitsSkippedNoAI;
					appendCombatOrderSample(noAIExamples, noAISampleCount, obj);
				}
			}
		}
	}

	BotAILog("%s | sendIdleCombatTeamsToGuardPosition (%.0f,%.0f): sent=%d  teams=%d(skip_notdeployable=%d locked=%d lockUntil=%u)  skipped non-combat=%d busy=%d no-ai=%d",
		getBotLogPrefix(m_player).str(), targetPos->x, targetPos->y,
		unitsSent, teamsTotal, teamsSkippedNotDeployable, teamsLocked, lockUntil,
		unitsSkippedNonCombat, unitsSkippedBusy, unitsSkippedNoAI);
	if (!issuedSamples.isEmpty())
	{
		BotAILog("%s |   sendIdleCombatTeamsToGuardPosition sample: %s",
			getBotLogPrefix(m_player).str(), issuedSamples.str());
	}
	if (unitsSkippedNoAI > 0 && !noAIExamples.isEmpty())
	{
		BotAILog("%s |   sendIdleCombatTeamsToGuardPosition no-ai sample: %s",
			getBotLogPrefix(m_player).str(), noAIExamples.str());
	}
	return unitsSent;
}
...
Int AISkirmishPlayer::sendAllCombatTeamsToPosition(const Coord3D *targetPos, const char *debugReason, Int relatedPlayerIndex)
{
	...
	// Lock every combat team for long enough that the 30s refresh comfortably re-applies it.
	const UnsignedInt curFrame = TheGameLogic ? TheGameLogic->getFrame() : 0;
	const UnsignedInt lockUntil = curFrame + TEAM_COMMAND_LOCK_FRAMES;
	...
	Int teamsSkippedNoOrders = 0;
	Int unitsSkippedNonCombat = 0;
	Int unitsSkippedAlreadyOutsideBreakout = 0;
	Int unitsSkippedNoAI = 0;
	AsciiString noAIExamples;
	Int noAISampleCount = 0;
	AsciiString issuedSamples;
	Int issuedSampleCount = 0;
	const Real breakoutCommandDistance = maxReal(m_baseRadius * GROUP_ATTACK_BREAKOUT_DISTANCE_SCALE,
		m_baseRadius + GROUP_ATTACK_BREAKOUT_DISTANCE_PADDING);
	const Real breakoutCommandDistanceSq = sqr(breakoutCommandDistance);
	const Real breakoutCloseEnoughSq = sqr(GROUP_ATTACK_BREAKOUT_POINT_CLOSE_ENOUGH);
	for (Player::PlayerTeamList::const_iterator teamProtoIt = m_player->getPlayerTeams()->begin();
		teamProtoIt != m_player->getPlayerTeams()->end();
		++teamProtoIt)
	{
		for (DLINK_ITERATOR<Team> iter = (*teamProtoIt)->iterate_TeamInstanceList(); !iter.done(); iter.advance())
		{
			...
			for (DLINK_ITERATOR<Object> objIter = team->iterate_TeamMemberList(); !objIter.done(); objIter.advance())
			{
				...
				if (objPos != nullptr)
				{
					if (distanceSquared2D(*objPos, m_baseCenter) > breakoutCommandDistanceSq
						|| distanceSquared2D(*objPos, *targetPos) <= breakoutCloseEnoughSq)
					{
						++unitsSkippedAlreadyOutsideBreakout;
						continue;
					}
				}
			...
			if (!issuedOrder)
			{
				++teamsSkippedNoOrders;
			}
		}
	}

	BotAILog("%s | sendAllCombatTeams to (%.0f,%.0f): sent=%d  teams=%d(skip_empty=%d skip_noorders=%d locked=%d lockUntil=%u)  skipped non-combat=%d no-ai=%d",
		getBotLogPrefix(m_player).str(), targetPos->x, targetPos->y,
		unitsSent, teamsTotal, teamsSkippedEmpty, teamsSkippedNoOrders, teamsLocked, lockUntil, unitsSkippedNonCombat, unitsSkippedNoAI);
	if (!issuedSamples.isEmpty())
	{
	...
	if (unitsSkippedNoAI > 0 && !noAIExamples.isEmpty())
	{
		BotAILog("%s |   sendAllCombatTeams no-ai sample: %s",
			getBotLogPrefix(m_player).str(), noAIExamples.str());
	}
	return unitsSent;
}
...
//----------------------------------------------------------------------------------------------------------
/**
 * Send ALL combat teams (not just idle ones) toward a target position with a pure move order.
 * Used by the coordinated breakout phase so pathing clears the base choke before attack-move engages.
 */
Int AISkirmishPlayer::sendAllCombatTeamsToMovePosition(const Coord3D *targetPos, const char *debugReason, Int relatedPlayerIndex)
{
	if (targetPos == nullptr)
	{
		BotAILog("%s | sendAllCombatTeamsToMovePosition called with null targetPos!", getBotLogPrefix(m_player).str());
		return 0;
	}

	const UnsignedInt curFrame = TheGameLogic ? TheGameLogic->getFrame() : 0;
	const UnsignedInt lockUntil = curFrame + TEAM_COMMAND_LOCK_FRAMES;
	const char *orderReason = resolveCombatOrderReason(debugReason, "all_move");
	...
	BotAILog("%s | sendAllCombatTeamsToMovePosition (%.0f,%.0f): sent=%d  teams=%d(skip_empty=%d skip_noorders=%d locked=%d lockUntil=%u)  skipped non-combat=%d outside-breakout=%d no-ai=%d",
		getBotLogPrefix(m_player).str(), targetPos->x, targetPos->y,
		unitsSent, teamsTotal, teamsSkippedEmpty, teamsSkippedNoOrders, teamsLocked, lockUntil, unitsSkippedNonCombat, unitsSkippedAlreadyOutsideBreakout, unitsSkippedNoAI);
	if (!issuedSamples.isEmpty())
	{
		BotAILog("%s |   sendAllCombatTeamsToMovePosition sample: %s",
			getBotLogPrefix(m_player).str(), issuedSamples.str());
	}
	if (unitsSkippedNoAI > 0 && !noAIExamples.isEmpty())
	{
		BotAILog("%s |   sendAllCombatTeamsToMovePosition no-ai sample: %s",
			getBotLogPrefix(m_player).str(), noAIExamples.str());
	}
	return unitsSent;
}
...
Int AISkirmishPlayer::sendAllCombatTeamsToGuardPosition(const Coord3D *targetPos, const char *debugReason, Int relatedPlayerIndex)
{
	...
	BotAILog("%s | sendAllCombatTeamsToGuardPosition (%.0f,%.0f): %d units in guard mode  teamsLocked=%d skip_noorders=%d lockUntil=%u  skipped non-combat=%d no-ai=%d",
		getBotLogPrefix(m_player).str(), targetPos->x, targetPos->y,
		unitsSent, teamsLocked, teamsSkippedNoOrders, lockUntil, unitsSkippedNonCombat, unitsSkippedNoAI);
	if (!issuedSamples.isEmpty())
	{
	...
	if (unitsSkippedNoAI > 0 && !noAIExamples.isEmpty())
	{
		BotAILog("%s |   sendAllCombatTeamsToGuardPosition no-ai sample: %s",
			getBotLogPrefix(m_player).str(), noAIExamples.str());
	}
	return unitsSent;
}
...
//----------------------------------------------------------------------------------------------------------
/**
 * Release the Team command-lock on all our combat teams so their per-frame scripted
 * state reactions (onIdle, onEnemySighted, generic scripts) resume immediately.
 */
void AISkirmishPlayer::clearAllCombatTeamCommandLocks()
{
	if (m_player == nullptr || m_player->getPlayerTeams() == nullptr)
		return;
		...
		++teamProtoIt)
	{
		for (DLINK_ITERATOR<Team> iter = (*teamProtoIt)->iterate_TeamInstanceList(); !iter.done(); iter.advance())
		{
			...
			if (team->getCommandLockUntilFrame() != 0)
			{
				team->setCommandLockUntilFrame(0);
				++cleared;
			}
		}
	}
	BotAILog("%s | clearAllCombatTeamCommandLocks -> cleared %d team locks", getBotLogPrefix(m_player).str(), cleared);
}
...
void AISkirmishPlayer::assistAlliedPlayers()
{
	...
	if (m_coordAttackBreakoutPending)
		return;
	...
	Int unitsSent = sendIdleCombatTeamsToGuardPosition(&targetPos,
		fromHumanRequest ? "assist_human_idle_guard" : "assist_auto_idle_guard",
		assistTarget ? assistTarget->getPlayerIndex() : PLAYER_INDEX_INVALID);
	if (unitsSent <= 0)
		return;
	BotAILog("%s | assistAlliedPlayers: guarding '%s' with %d idle units at (%.0f,%.0f) (humanReq=%d)",
		getBotLogPrefix(m_player).str(), getPlayerNameAscii(assistTarget).str(), unitsSent, targetPos.x, targetPos.y, (Int)fromHumanRequest);

	AsciiString msg;
	msg.format("Sending support to %s.", getPlayerDisplayLabel(assistTarget).str());
	sendBotChat(msg, true, false);
}
...
void AISkirmishPlayer::coordinateGroupAttack()
{
	...
	if (m_coordAttackBreakoutPending)
	{
		serviceCoordinatedAttackBreakout();
		return;
	}
	...
	if (leadAI != nullptr && leadAI != this)
	{
		if (!m_coordAttackPending && !hasIdleCombatTeams())
		{
			if (curFrame >= m_frameNextCoordSkipLog)
			{
				m_frameNextCoordSkipLog = curFrame + 30 * LOGICFRAMES_PER_SECOND;
				BotAILog("%s | GROUP: skip join-lead (no idle combat teams)", getBotLogPrefix(m_player).str());
			}
			return;
		}

		// Don't join a lead bot's attack if we recently received a human command (3-min lock-in).
		if (curFrame < m_frameLastPlanChange + PLAN_CHANGE_COOLDOWN_FRAMES)
		{
			DEBUG_LOG(("BotAI [%s]: coordinateGroupAttack -> skipping join-lead (human command cooldown %d frames remaining)",
				getPlayerNameAscii(m_player).str(), (Int)(m_frameLastPlanChange + PLAN_CHANGE_COOLDOWN_FRAMES - curFrame)));
		}
		else
		{
			...
			// Only log to the BotAI file the FIRST time we join a particular lead, not every 2s while we wait.
			const Int leadIdx = leadAI->m_player ? leadAI->m_player->getPlayerIndex() : -1;
			if (leadIdx != m_coordAttackLastLoggedLeadIdx)
			{
				m_coordAttackLastLoggedLeadIdx = leadIdx;
				BotAILog("%s | GROUP: joining lead bot '%s' attack on '%s'  target=(%.0f,%.0f)",
					...
					leadEnemy ? getPlayerColorName(leadEnemy).str() : "?",
					leadTarget.x, leadTarget.y);
			}
			queueCoordinatedAttack(&leadTarget, leadEnemy, leadWaitStart);
		}
	}
	else if (!m_coordAttackPending && enemy != nullptr)
	{
		// Self-initiate only with idle teams. Pulling active attack groups back to a rally
		// point was the source of the "attack, then run home, then attack again" loop.
		// If no ally is immediately available, still start staging and use the normal
		// timeout fallback so bots don't become permanently passive.
		if (!hasIdleCombatTeams())
		{
			if (curFrame >= m_frameNextCoordSkipLog)
			{
				m_frameNextCoordSkipLog = curFrame + 30 * LOGICFRAMES_PER_SECOND;
				BotAILog("%s | GROUP: skip self-init (no idle combat teams)", getBotLogPrefix(m_player).str());
			}
			return;
		}
		...
		if (curFrame < m_frameLastPlanChange + PLAN_CHANGE_COOLDOWN_FRAMES)
		{
			// Throttle this log aggressively — previously it spammed once every 2s per bot.
			// Only log once per 30s of waiting so we can see WHY bots aren't grouping,
			// without filling the log.
			if (curFrame >= m_frameNextCoordSkipLog)
			{
				m_frameNextCoordSkipLog = curFrame + 30 * LOGICFRAMES_PER_SECOND;
				BotAILog("%s | GROUP: skip self-init (plan cooldown %u frames left)",
					getBotLogPrefix(m_player).str(),
					(unsigned)(m_frameLastPlanChange + PLAN_CHANGE_COOLDOWN_FRAMES - curFrame));
			}
			return;
		}
		...
		for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i)
		{
			...
			AISkirmishPlayer *allyAI = static_cast<AISkirmishPlayer *>(otherPlayer->getAIPlayer());
			if (allyAI == nullptr || allyAI->isBaseUnderAttack() || !allyAI->hasIdleCombatTeams())
			{
				continue;
			}
		...
		if (eligibleAllies <= 0)
		{
			BotAILog("%s | GROUP: self-init with no currently eligible allies -> staging anyway and waiting up to %u frames for timeout fallback",
				getBotLogPrefix(m_player).str(), (unsigned)GROUP_ATTACK_WAIT_FRAMES);
		}

		DEBUG_LOG(("BotAI [%s]: coordinateGroupAttack -> self-initiating attack on '%s'",
			getPlayerNameAscii(m_player).str(), getPlayerNameAscii(enemy).str()));
		BotAILog("%s | GROUP: self-initiating attack on '%s'  eligibleAllies=%d",
			getBotLogPrefix(m_player).str(), getPlayerColorName(enemy).str(), eligibleAllies);
		Coord3D targetPos;
		Region2D bounds;
		getPlayerStructureBounds(&bounds, enemy->getPlayerIndex());
		targetPos.x = bounds.lo.x + bounds.width() / 2;
	...
	for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i)
	{
		...
		if (allyAI->m_coordAttackBreakoutPending)
		{
			continue;
		}
		if (!allyAI->m_coordAttackPending && !allyAI->hasIdleCombatTeams())
		{
			continue;
		}
	...
	for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i)
	{
		...
		if (!allyAI->m_coordAttackPending
			&& (curFrame < allyAI->m_frameLastPlanChange + PLAN_CHANGE_COOLDOWN_FRAMES
				|| allyAI->m_holdAttackUntilFrame > curFrame
				|| (allyAI->m_requestedAssistUntilFrame > curFrame && allyAI->m_requestedAssistPlayerIndex != PLAYER_INDEX_INVALID)))
		{
			continue;
		...
		if ((!allyAI->m_coordAttackPending && !allyAI->m_coordAttackBreakoutPending)
			|| allyAI->m_coordAttackEnemyPlayerIndex != coordEnemy->getPlayerIndex())
		{
	...
	Bool readyToLaunch = timeExpired || (alliedBotsConsidered > 0 && allAlliesReady);
	BotAILog("%s | GROUP: staging  enemy='%s'  target=(%.0f,%.0f)  elapsed=%u/%u  alliesConsidered=%d  allReady=%d  timeExpired=%d  readyToLaunch=%d",
		...
		alliedBotsConsidered, (int)allAlliesReady, (int)timeExpired, (int)readyToLaunch);
	if (!readyToLaunch)
	{
		...
		rallyTeamsToStagingPoint("group_wait_rally", m_coordAttackEnemyPlayerIndex);
		return;
	}
	...
	for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i)
	{
		...
		const Bool allyAlreadyStagingThisAttack = allyAI->m_coordAttackPending
			&& allyAI->m_coordAttackEnemyPlayerIndex == coordEnemy->getPlayerIndex();
		const Bool allyAlreadyLaunchedThisAttack = allyAI->m_coordAttackBreakoutPending
			&& allyAI->m_coordAttackEnemyPlayerIndex == coordEnemy->getPlayerIndex();
		if (allyAlreadyLaunchedThisAttack)
		{
			continue;
		}
		if (!allyAlreadyStagingThisAttack && !allyAI->hasIdleCombatTeams())
		{
			continue;
		}
		// Don't force-launch allies that are locked into a different recent plan, but do
		// launch allies already staged for this exact attack.
		if (!allyAlreadyStagingThisAttack
			&& (curFrame < allyAI->m_frameLastPlanChange + PLAN_CHANGE_COOLDOWN_FRAMES
				|| allyAI->m_holdAttackUntilFrame > curFrame
				|| (allyAI->m_requestedAssistUntilFrame > curFrame && allyAI->m_requestedAssistPlayerIndex != PLAYER_INDEX_INVALID)))
		{
...
void AISkirmishPlayer::launchCoordinatedAttack(const Coord3D *targetPos, const Player *enemy, Bool dueToTimeout)
{
	...
	m_frameLastPlanChange = curFrame;
	m_frameNextCoordSkipLog = 0;
	if (hasNonZero2D(m_coordBreakoutPoint))
	{
		Int breakoutSent = sendAllCombatTeamsToMovePosition(&m_coordBreakoutPoint,
			dueToTimeout ? "group_launch_breakout_timeout" : "group_launch_breakout",
			enemy->getPlayerIndex());
		BotAILog("%s | GROUP: breakout launch -> %d units move to (%.0f,%.0f) before attack target (%.0f,%.0f) timeout=%d",
			getBotLogPrefix(m_player).str(), breakoutSent,
			m_coordBreakoutPoint.x, m_coordBreakoutPoint.y,
		...
		if (breakoutSent > 0)
		{
			...
			m_coordAttackPending = false;
			m_coordAttackBreakoutPending = true;
			m_coordAttackWaitStartFrame = 0;
			m_coordAttackBreakoutUntilFrame = curFrame + GROUP_ATTACK_BREAKOUT_FRAMES;
			m_holdAttackUntilFrame = 0;
			m_coordRallyPoint.zero();
			m_coordAttackLastLoggedLeadIdx = -1;
			return;
	...
	Int unitsSent = sendAllCombatTeamsToPosition(targetPos,
		dueToTimeout ? "group_launch_timeout" : "group_launch_ready",
		enemy->getPlayerIndex());
	if (unitsSent > 0)
	{
		AsciiString msg;
...
//----------------------------------------------------------------------------------------------------------
/**
 * Keep a freshly launched coordinated attack in move-only breakout long enough to clear the base choke,
 * then release it to the normal attack-move order.
 */
void AISkirmishPlayer::serviceCoordinatedAttackBreakout()
{
	if (!m_coordAttackBreakoutPending)
	{
		return;
	...
	if (!hasNonZero2D(m_coordBreakoutPoint) || !hasNonZero2D(m_coordAttackTarget)
		|| m_coordAttackEnemyPlayerIndex == PLAYER_INDEX_INVALID)
	{
		BotAILog("%s | GROUP: breakout state incomplete -> cancelling", getBotLogPrefix(m_player).str());
		cancelCoordinatedAttack();
		return;
	}

	Player *enemy = ThePlayerList ? ThePlayerList->getNthPlayer(m_coordAttackEnemyPlayerIndex) : nullptr;
	if (enemy == nullptr || !enemy->hasAnyObjects())
	{
		BotAILog("%s | GROUP: breakout target invalid -> cancelling", getBotLogPrefix(m_player).str());
		cancelCoordinatedAttack();
		return;
	}

	const UnsignedInt curFrame = TheGameLogic ? TheGameLogic->getFrame() : 0;
	const Real releaseDistance = maxReal(m_baseRadius * GROUP_ATTACK_RELEASE_DISTANCE_SCALE,
		m_baseRadius + GROUP_ATTACK_RELEASE_DISTANCE_PADDING);
	const Real releaseDistanceSq = sqr(releaseDistance);
	Bool reachedBreakout = false;
	Real sampleBaseDistSq = 0.0f;
	Real sampleBreakoutDistSq = 0.0f;

	Object *sampleUnit = findRepresentativeGroundCombatUnit(m_player, m_baseCenter);
	if (sampleUnit != nullptr && sampleUnit->getPosition() != nullptr)
	{
		sampleBaseDistSq = distanceSquared2D(*sampleUnit->getPosition(), m_baseCenter);
		sampleBreakoutDistSq = distanceSquared2D(*sampleUnit->getPosition(), m_coordBreakoutPoint);
		reachedBreakout = sampleBaseDistSq >= releaseDistanceSq
			|| sampleBreakoutDistSq <= sqr(GROUP_ATTACK_BREAKOUT_POINT_CLOSE_ENOUGH);
	}

	const Bool breakoutTimedOut = curFrame >= m_coordAttackBreakoutUntilFrame;
	if (!reachedBreakout && !breakoutTimedOut)
	{
		BotAILog("%s | GROUP: breakout holding at (%.0f,%.0f)  sampleBaseDist=%.0f  sampleBreakoutDist=%.0f  releaseAt=%u",
			getBotLogPrefix(m_player).str(),
			m_coordBreakoutPoint.x,
			m_coordBreakoutPoint.y,
			sqrt(sampleBaseDistSq),
			sqrt(sampleBreakoutDistSq),
			m_coordAttackBreakoutUntilFrame);
		return;
	}

	Int unitsSent = sendAllCombatTeamsToPosition(&m_coordAttackTarget,
		breakoutTimedOut ? "group_breakout_release_timeout" : "group_breakout_release",
		m_coordAttackEnemyPlayerIndex);
	BotAILog("%s | GROUP: breakout release -> %d units attack-move to (%.0f,%.0f)  reached=%d timeout=%d sampleBaseDist=%.0f sampleBreakoutDist=%.0f",
		getBotLogPrefix(m_player).str(),
		unitsSent,
		m_coordAttackTarget.x,
		m_coordAttackTarget.y,
		(Int)reachedBreakout,
		(Int)breakoutTimedOut,
		sqrt(sampleBaseDistSq),
		sqrt(sampleBreakoutDistSq));
	cancelCoordinatedAttack();
}
...
Player *AISkirmishPlayer::findPlayerByColorName(const AsciiString &command) const
{
	...
	{
		Player *candidate = ThePlayerList->getNthPlayer(i);
		// Allow self: "move yellow" said to the yellow bot should resolve to Yellow (i.e. go home).
		if (candidate == nullptr || !candidate->hasAnyObjects())
			continue;
...
/** Xfer method
	...
	* 4: Human-command attack/guard distinction
	* 5: Coordinated-attack breakout state */
// ------------------------------------------------------------------------------------------------
void AISkirmishPlayer::xfer( Xfer *xfer )
{
	...
	if (version >= 4)
	{
		xfer->xferBool(&m_humanCommandIsAttack);
		xfer->xferInt(&m_humanCommandAttackEnemyIndex);
	}

	if (version >= 5)
	{
		xfer->xferUnsignedInt(&m_coordAttackBreakoutUntilFrame);
		xfer->xferCoord3D(&m_coordBreakoutPoint);
		xfer->xferBool(&m_coordAttackBreakoutPending);
	}
...
void AISkirmishPlayer::loadPostProcess()
{
	...
	if (!m_coordAttackPending && !m_coordAttackBreakoutPending)
	{
		cancelCoordinatedAttack();
	}
	...
	if (m_humanCommandAttackEnemyIndex < PLAYER_INDEX_INVALID || m_humanCommandAttackEnemyIndex >= MAX_PLAYER_COUNT)
	{
		m_humanCommandAttackEnemyIndex = PLAYER_INDEX_INVALID;
	}
	...
	if (m_coordAttackBreakoutPending)
	{
		if (m_coordAttackEnemyPlayerIndex == PLAYER_INDEX_INVALID || !hasNonZero2D(m_coordBreakoutPoint) || m_coordAttackBreakoutUntilFrame == 0)
		{
			cancelCoordinatedAttack();
		}
	}

	if (m_holdAttackUntilFrame == 0)
	{
		m_humanCommandIsAttack = false;
		m_humanCommandAttackEnemyIndex = PLAYER_INDEX_INVALID;
	}

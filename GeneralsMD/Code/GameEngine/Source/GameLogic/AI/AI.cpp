...
void AI::update()
{
	...
	#define AI_PERF_BEGIN() QueryPerformanceCounter(&_qpcA)
	#define AI_PERF_END(tag) do { QueryPerformanceCounter(&_qpcB); \
		if (kMsPerTick > 0.0) PerfRecordLogicSub((tag), (double)(_qpcB.QuadPart - _qpcA.QuadPart) * kMsPerTick); } while(0)

	// Service pathfinding.  Eligible ordinary move A* jobs are submitted to the
	// persistent worker; completed worker paths are applied here on the main thread.
#if defined(GENERALS_ONLINE_HIGH_FPS_SERVER)
	if (TheGameLogic->HasLegacyFrameAdvanced())
	{
#endif
		AI_PERF_BEGIN();
		m_pathfinder->serviceAsyncPathfindQueue();
		AI_PERF_END("Lg_AI_PathSvc");
#if defined(GENERALS_ONLINE_HIGH_FPS_SERVER)
	}
#endif

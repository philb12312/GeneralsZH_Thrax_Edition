...
namespace
{
	class InGameUIPerfScope
	{
	public:
		explicit InGameUIPerfScope(const char *tag) : m_token(PerfBeginScopedLogicSub(tag)) {}
		~InGameUIPerfScope() { PerfEndScopedLogicSub(m_token); }

	private:
		int m_token;
	};

	#define UI_PERF_JOIN2(a, b) a##b
	#define UI_PERF_JOIN(a, b) UI_PERF_JOIN2(a, b)
	#define UI_PERF_SCOPE(tag) InGameUIPerfScope UI_PERF_JOIN(_uiPerfScope_, __LINE__)(tag)
}
...
#pragma MESSAGE("*** WARNING *** DOING DO_UNIT_TIMINGS!!!!")
	...
	{
		UI_PERF_SCOPE("Cl_UI_Win");
		TheWindowManager->winRepaint();
	}

	{
		UI_PERF_SCOPE("Cl_UI_PostWin");
		postWindowDraw();
	}

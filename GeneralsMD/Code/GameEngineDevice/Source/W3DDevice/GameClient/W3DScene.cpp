...
// Per-subsystem perf instrumentation hooks (implemented in GameEngine.cpp)
...
namespace
{
	class W3DScenePerfScope
	{
	public:
		W3DScenePerfScope(const char *tag) :
	...
	#define W3D_PERF_JOIN2(a, b) a##b
	#define W3D_PERF_JOIN(a, b) W3D_PERF_JOIN2(a, b)
	#define W3D_SCENE_PERF_SCOPE(tag) W3DScenePerfScope W3D_PERF_JOIN(_w3dScenePerfScope_, __LINE__)(tag)

	int g_scenePerfVisibilityVisited = 0;
	int g_scenePerfVisibilityVisible = 0;
	int g_scenePerfVisibilityHidden = 0;
	int g_scenePerfVisibilityTranslucent = 0;
	int g_scenePerfVisibilityDelayed = 0;
	int g_scenePerfPotentialOccluders = 0;
	int g_scenePerfPotentialOccludees = 0;

	const char *ScenePerfDrawableTemplateName(Drawable *draw)
	{
		if (draw == nullptr || draw->getTemplate() == nullptr)
			return "?";
		return draw->getTemplate()->getName().str();
	}
}
...
void RTS3DScene::Visibility_Check(CameraClass * camera)
{
	...
	m_translucentObjectsCount=0;
	m_numNonOccluderOrOccludee=0;
	g_scenePerfVisibilityVisited = 0;
	g_scenePerfVisibilityVisible = 0;
	g_scenePerfVisibilityHidden = 0;
	g_scenePerfVisibilityTranslucent = 0;
	g_scenePerfVisibilityDelayed = 0;
	g_scenePerfPotentialOccluders = 0;
	g_scenePerfPotentialOccludees = 0;
	...
	if (ShaderClass::Is_Backface_Culling_Inverted())
	{
		...
		// Loop over all top-level RenderObjects in this scene.
		for (it.First(); !it.Is_Done(); it.Next()) {

			robj = it.Peek_Obj();
			++g_scenePerfVisibilityVisited;
			...
			if (robj->Is_Really_Visible())
				++g_scenePerfVisibilityVisible;
			else
				++g_scenePerfVisibilityHidden;
		}
	}
	else
	{
            ...
            if ( ! isVisible )
			{
              ...
              continue;
						...
						if (draw->getEffectiveOpacity() != 1.0f && m_translucentObjectsCount < TheGlobalData->m_maxVisibleTranslucentObjects)
						{
							...
							++g_scenePerfVisibilityTranslucent;
						}
	...
	g_scenePerfPotentialOccluders = m_numPotentialOccluders;
	g_scenePerfPotentialOccludees = m_numPotentialOccludees;
	g_scenePerfVisibilityDelayed = m_numPotentialOccluders + m_numPotentialOccludees + m_numNonOccluderOrOccludee;
...
void RTS3DScene::renderOneObject(RenderInfoClass &rinfo, RenderObjClass *robj, Int localPlayerIndex)
{
	...
	W3D_SCENE_PERF_SCOPE("Cl_Obj_Total");

	if (robj->Class_ID() == RenderObjClass::CLASSID_IMAGE3D	)
	{
		{
			W3D_SCENE_PERF_SCOPE("Cl_Obj_RObj");
	...
	if (draw && (drawableHidden=draw->isDrawableEffectivelyHidden()) != TRUE)
	{
		...
		if (obj) {
			{
				W3D_SCENE_PERF_SCOPE("Cl_Obj_Shroud");
		...
		{
			W3D_SCENE_PERF_SCOPE("Cl_Obj_Lights");
	...
	{
		{
			W3D_SCENE_PERF_SCOPE("Cl_Obj_DynLight");
		...
		{
			W3D_SCENE_PERF_SCOPE("Cl_Obj_PreLight");
...
void RTS3DScene::Flush(RenderInfoClass & rinfo)
{
	...
	{
		W3D_SCENE_PERF_SCOPE("Cl_Fl_Prep");
		PrepareShadows();
	}
	...
	{
		W3D_SCENE_PERF_SCOPE("Cl_Fl_Occl");
#ifdef USE_NON_STENCIL_OCCLUSION
	...
	{
		{
			W3D_SCENE_PERF_SCOPE("Cl_Fl_ShVol");
			DoShadows(rinfo, true);	//draw all stencil shadows
		}
	}

	{
		W3D_SCENE_PERF_SCOPE("Cl_Fl_Water");
	...
	{
		{
			W3D_SCENE_PERF_SCOPE("Cl_Fl_Trans");
...

	{
		...
		{
			{
				W3D_SCENE_PERF_SCOPE("Cl_Fl_Parts");
				DoParticles(rinfo);	//queue up particles for rendering.
			}
		}

		{
			W3D_SCENE_PERF_SCOPE("Cl_Fl_Sort");
...
void RTS3DScene::Render(RenderInfoClass & rinfo)
{
	...
	if (Get_Extra_Pass_Polygon_Mode() == EXTRA_PASS_DISABLE)
	{
		if (m_customPassMode == SCENE_PASS_DEFAULT)
		{
			...
			{
				W3D_SCENE_PERF_SCOPE("Cl_Cu_Render");
...
#define USE_LIGHT_ENV 1
   ...
   if (!Visibility_Checked) {
      // set the visibility bit in all render objects in all layers.
	   {
		   W3D_SCENE_PERF_SCOPE("Cl_Cu_VisCheck");
		   Visibility_Check(&rinfo.Camera);
	   }
#ifdef USE_NON_STENCIL_OCCLUSION
	   {
		   W3D_SCENE_PERF_SCOPE("Cl_Cu_OccludeFlag");
		   flagOccludedObjects(&rinfo.Camera);
	   }
#endif
   }
   Visibility_Checked = false;
	PerfRecordRenderFeatureStats("SceneVis", 0, g_scenePerfVisibilityVisited, g_scenePerfVisibilityVisible,
		g_scenePerfVisibilityHidden, g_scenePerfVisibilityTranslucent, g_scenePerfVisibilityDelayed);
	PerfRecordRenderFeatureStats("SceneOcc", 0, g_scenePerfPotentialOccluders, g_scenePerfPotentialOccludees,
		m_occludedObjectsCount, m_numNonOccluderOrOccludee, TheGlobalData ? TheGlobalData->m_maxVisibleOccludeeObjects : 0);
	...
	{
		W3D_SCENE_PERF_SCOPE("Cl_Cu_OnFrame");
		for (it.First(); !it.Is_Done(); it.Next()) {
			RenderObjClass * robj = it.Peek_Obj();
			++updateVisited;
	...
	PerfRecordRenderFeatureStats("SceneUpd", 0, updateVisited, terrainObject ? 1 : 0, 0, 0, 0);

	//terrain needs to be rendered first
	if (terrainObject)	// Don't check visibility - terrain is always visible. jba.
	{
		W3D_SCENE_PERF_SCOPE("Cl_Cu_Terrain");
		robj=terrainObject;
	...
	// loop through all render objects in the list:
	{
		W3D_SCENE_PERF_SCOPE("Cl_Cu_ObjLoop");
		const double objMsPerTick = 0.0;
		int renderVisited = 0;
		int renderVisible = 0;
		int renderDelayed = 0;
		int renderObjects = 0;
...
#endif
				{
					LARGE_INTEGER objA, objB;
					if (objMsPerTick > 0.0)
						QueryPerformanceCounter(&objA);
					renderOneObject(rinfo, robj, localPlayerIndex);
					if (objMsPerTick > 0.0)
					{
						QueryPerformanceCounter(&objB);
						const double objMs = (double)(objB.QuadPart - objA.QuadPart) * objMsPerTick;
						PerfRecordNamedCost("ObjTpl", ScenePerfDrawableTemplateName(draw), objMs);
						PerfRecordNamedCost("RObj", robj->Get_Name() ? robj->Get_Name() : "?", objMs);
					}
					++renderObjects;
				}
				else
		...
		PerfRecordRenderFeatureStats("SceneObj", 0, renderVisited, renderObjects, renderVisible, renderDelayed, updateVisited);
	}
	...
	{
		W3D_SCENE_PERF_SCOPE("Cl_Cu_QueueShadow");
	...
	if (terrainObject != nullptr && TheParticleSystemManager != nullptr &&
		Get_Extra_Pass_Polygon_Mode() == EXTRA_PASS_DISABLE)
	{
		W3D_SCENE_PERF_SCOPE("Cl_Cu_QueueParticle");
...
void RTS3DScene::flushOccludedObjectsIntoStencil(RenderInfoClass & rinfo)
{
	...
	// TheSuperHackers @perf (GameClientMCP) 2026-04-17: Only run the expensive stencil
	// occlusion pipeline when at least one occludee is ACTUALLY behind an occluder.
	// The original gate (m_numPotentialOccludees && m_numPotentialOccluders) triggered
	// the 3-pass + per-player-color stencil work for every frame that had both units
	// and buildings visible, even when nothing was actually occluded. flagOccludedObjects()
	// (called earlier in Customized_Render via Visibility_Check path) sets
	// m_occludedObjectsCount to the true count. When zero, the visible output is
	// byte-identical to the simple single-flush path below. Saves typically 1-2ms of
	// state juggling per frame in normal play and restores headroom in heavy fights
	// where only a handful of units are actually behind buildings.
	if (m_occludedObjectsCount > 0)
	{
		// TheSuperHackers @perf (GameClientMCP) 2026-04-17: sub-instrument the 4 stencil
		// occlusion passes so the next perf log tells us which one owns Cl_Fl_Occl's ~2.9 ms.
		const double _ocMs = PerfGetFreqQuadPartAsMsMultiplier();
		LARGE_INTEGER _ocA = {}, _ocB = {};
		if (_ocMs > 0.0)
			QueryPerformanceCounter(&_ocA);
		...
		if (_ocMs > 0.0)
		{
			QueryPerformanceCounter(&_ocB);
			PerfRecordLogicSub("Cl_Oc_PassA", (double)(_ocB.QuadPart - _ocA.QuadPart) * _ocMs);
			_ocA = _ocB;
		}
		...
		{
			QueryPerformanceCounter(&_ocB);
			PerfRecordLogicSub("Cl_Oc_PassB", (double)(_ocB.QuadPart - _ocA.QuadPart) * _ocMs);
			_ocA = _ocB;
		}
		...
		{
			QueryPerformanceCounter(&_ocB);
			PerfRecordLogicSub("Cl_Oc_PassC", (double)(_ocB.QuadPart - _ocA.QuadPart) * _ocMs);
			_ocA = _ocB;
		}
		...
		{
			QueryPerformanceCounter(&_ocB);
			PerfRecordLogicSub("Cl_Oc_PassD", (double)(_ocB.QuadPart - _ocA.QuadPart) * _ocMs);
		}
	}
	else
	if (m_numNonOccluderOrOccludee || m_numPotentialOccluders || m_numPotentialOccludees)
	{
		// Fast path - no occluded objects, skip stencil state juggling entirely.
		W3D_SCENE_PERF_SCOPE("Cl_Oc_Fast");

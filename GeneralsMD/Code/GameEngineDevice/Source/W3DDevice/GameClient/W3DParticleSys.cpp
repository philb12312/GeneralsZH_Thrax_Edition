...
static inline Bool ParticleSphereCulledByPlanes(const PlaneClass *planes, const Coord3D *pos, Real radius)
{
	for (Int i = 0; i < 6; ++i)
	{
		const PlaneClass &plane = planes[i];
		if ((pos->x * plane.N.X + pos->y * plane.N.Y + pos->z * plane.N.Z - plane.D) > radius)
			return TRUE;
	}
	return FALSE;
}
...
void W3DParticleSystemManager::doParticles(RenderInfoClass &rinfo)
{
	...
	const PlaneClass *frustumPlanes = frustum.Planes;
	...
	for( ParticleSystemManager::ParticleSystemListIt it = particleSysList.begin(); it != particleSysList.end(); ++it)
	{
		...
		// TheSuperHackers @perf (GameClientMCP) 2026-04-17: fast-out empty systems.
		...
		if (/*sys->isUsingSmudge()*/ *((DWORD *)sys->getParticleTypeName().str()) == 0x44554D53)
		{
			...
			{
				const ParticlePriorityType priority = sys->getPriority();
				const Bool cullParticles = (priority != ALWAYS_RENDER);
				...
				for (Particle *p = sys->getFirstParticle(); p; p = p->m_systemNext)
				{
					...
					if (cullParticles && ParticleSphereCulledByPlanes(frustumPlanes, pos, psize * 2.0f + 10.0f))
						continue;
		...
		const ParticlePriorityType priority = sys->getPriority();
		const Bool cullParticles = (priority != ALWAYS_RENDER);
		const Bool countFieldParticles = (priority == AREA_EFFECT && sys->m_isGroundAligned != FALSE);
		for (Particle *p = sys->getFirstParticle(); p; p = p->m_systemNext)
		{
			pos = p->getPosition();
			psize = p->getSize();
			...
			m_fieldParticleCount += countFieldParticles;

...
#define noLOG_STATS
	...
	// Push roads slightly toward the camera in the depth buffer so they always win the
	// Z test against the terrain directly beneath them. Fixes the classic "road polygons
	// flicker / go transparent when zoomed out" symptom, which is Z-fighting between
	// coplanar road and terrain quads once floating-point depth precision collapses at
	// high camera distance. D3DRS_ZBIAS range is 0..16; 8 is enough to dominate the test
	// without visibly detaching roads from the ground at low zoom.
	...
	// Restore default ZBIAS so nothing else in the frame is shifted.

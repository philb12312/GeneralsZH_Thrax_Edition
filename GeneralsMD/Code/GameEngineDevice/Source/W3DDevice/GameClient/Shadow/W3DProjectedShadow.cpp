...
/*						if (flipForBlend)
		...
		DX8Wrapper::_Set_DX8_Transform(D3DTS_WORLD, *(D3DMATRIX *)&mWorld);

		DX8Wrapper::_Set_DX8_Vertex_Buffer(0,shadowVertexBufferD3D,sizeof(SHADOW_VOLUME_VERTEX));
		m_pDev->SetVertexShader(SHADOW_VOLUME_FVF);

		Int numPolys = (endX - startX)*(endY - startY)*2;	//2 triangles per cell

		DWORD savedAlphaTestEnable;
		DWORD savedStencilEnable;
		DWORD savedStencilFunc;
		DWORD savedStencilRef;
		DWORD savedStencilMask;
		DWORD savedStencilWriteMask;
		DWORD savedStencilZFail;
		DWORD savedStencilFail;
		DWORD savedStencilPass;
		DWORD savedLighting;
		DWORD savedSrcBlend;
		DWORD savedDestBlend;
		m_pDev->GetRenderState(D3DRS_ALPHATESTENABLE, &savedAlphaTestEnable);
		m_pDev->GetRenderState(D3DRS_STENCILENABLE, &savedStencilEnable);
		m_pDev->GetRenderState(D3DRS_STENCILFUNC, &savedStencilFunc);
		m_pDev->GetRenderState(D3DRS_STENCILREF, &savedStencilRef);
		m_pDev->GetRenderState(D3DRS_STENCILMASK, &savedStencilMask);
		m_pDev->GetRenderState(D3DRS_STENCILWRITEMASK, &savedStencilWriteMask);
		m_pDev->GetRenderState(D3DRS_STENCILZFAIL, &savedStencilZFail);
		m_pDev->GetRenderState(D3DRS_STENCILFAIL, &savedStencilFail);
		m_pDev->GetRenderState(D3DRS_STENCILPASS, &savedStencilPass);
		m_pDev->GetRenderState(D3DRS_LIGHTING, &savedLighting);
		m_pDev->GetRenderState(D3DRS_SRCBLEND, &savedSrcBlend);
		m_pDev->GetRenderState(D3DRS_DESTBLEND, &savedDestBlend);
...
//    m_pDev->SetRenderState( D3DRS_ALPHABLENDENABLE, FALSE );	//useful to see bounds
		...
		m_pDev->SetRenderState(D3DRS_ALPHATESTENABLE, savedAlphaTestEnable);
		m_pDev->SetRenderState( D3DRS_STENCILENABLE, savedStencilEnable );
		m_pDev->SetRenderState( D3DRS_STENCILFUNC, savedStencilFunc );
		m_pDev->SetRenderState( D3DRS_STENCILREF, savedStencilRef );
		m_pDev->SetRenderState( D3DRS_STENCILMASK, savedStencilMask );
		m_pDev->SetRenderState( D3DRS_STENCILWRITEMASK, savedStencilWriteMask );
		m_pDev->SetRenderState( D3DRS_STENCILZFAIL, savedStencilZFail );
		m_pDev->SetRenderState( D3DRS_STENCILFAIL, savedStencilFail );
		m_pDev->SetRenderState( D3DRS_STENCILPASS, savedStencilPass );
//    m_pDev->SetRenderState( D3DRS_ALPHABLENDENABLE, TRUE );
		m_pDev->SetRenderState( D3DRS_LIGHTING, savedLighting);
		m_pDev->SetRenderState( D3DRS_SRCBLEND, savedSrcBlend);
		m_pDev->SetRenderState( D3DRS_DESTBLEND, savedDestBlend );
...
*/
	...
	DX8Wrapper::_Set_DX8_Index_Buffer(shadowDecalIndexBufferD3D,nShadowDecalStartBatchVertex);
	DX8Wrapper::_Set_DX8_Transform(D3DTS_WORLD, *(D3DMATRIX *)&mWorld);

	DX8Wrapper::_Set_DX8_Vertex_Buffer(0,shadowDecalVertexBufferD3D,sizeof(SHADOW_DECAL_VERTEX));
...
*/
		...
		if(pvVertices)
		{
			...
			for (j=startY; j <= endY; j++)
			{
				...
				{
					hmapVertex.X=(float)(i-borderSize)*MAP_XY_FACTOR;
					hmapVertex.Z=(float)hmap->getHeight(i,j)*MAP_HEIGHT_SCALE+0.2f * MAP_XY_FACTOR;
					pvVertices->x=hmapVertex.X;
					pvVertices->y=hmapVertex.Y;
					pvVertices->z=hmapVertex.Z;
					pvVertices->diffuse=shadow->m_diffuse;

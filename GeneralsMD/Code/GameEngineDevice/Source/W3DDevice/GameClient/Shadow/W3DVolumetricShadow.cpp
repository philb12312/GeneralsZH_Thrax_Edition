...
// TheSuperHackers @perf (GameClientMCP) 2026-04-17: perf sink for stencil shadow sub-breakdown.
...
void W3DVolumetricShadow::RenderVolume(Int meshIndex, Int lightIndex)
{
	...
	if (mesh)
	{
		...
		RenderMeshVolumeBounds(meshIndex, lightIndex, &meshTransform);
#endif
		if (m_shadowVolume[0][meshIndex]->GetFlags() & SHADOW_DYNAMIC)
			RenderDynamicMeshVolume(meshIndex, lightIndex, &meshTransform);
		else
			RenderMeshVolume(meshIndex, lightIndex, &meshTransform);
	}
}
...
void W3DVolumetricShadow::RenderMeshVolume(Int meshIndex, Int lightIndex, const Matrix3D* meshXform)
{
	...
	DX8Wrapper::_Set_DX8_Transform(D3DTS_WORLD, *(D3DMATRIX*)&mWorldTransposed);
	...
	DX8Wrapper::_Set_DX8_Vertex_Buffer(0, lastActiveVertexBuffer,
	...
	DX8Wrapper::_Set_DX8_Index_Buffer(ibSlot->m_IB->m_DX8IndexBuffer->Get_DX8_Index_Buffer(), vbSlot->m_start);
...
void W3DVolumetricShadow::RenderDynamicMeshVolume(Int meshIndex, Int lightIndex, const Matrix3D* meshXform)
{
	...
	DX8Wrapper::_Set_DX8_Index_Buffer(shadowIndexBufferD3D, nShadowStartBatchVertex);

	Matrix4x4 mWorld(*meshXform);
	Matrix4x4 mWorldTransposed = mWorld.Transpose();
	DX8Wrapper::_Set_DX8_Transform(D3DTS_WORLD, *(D3DMATRIX*)&mWorldTransposed);

	DX8Wrapper::_Set_DX8_Vertex_Buffer(0, shadowVertexBufferD3D, sizeof(SHADOW_DYNAMIC_VOLUME_VERTEX));
...
//	if( currentTime - lastTime >= delay )
	{
		...
		// TheSuperHackers @perf (GameClientMCP) 2026-04-17: EARLY bbox cull before the
		// expensive TheTerrainLogic->getGroundHeight() sample. Originally the ground-height
		// query ran for every enabled shadow caster every frame (50-200 of them in a heavy
		// fight) regardless of visibility. This test uses the MAX possible shadow extent
		// (airborne-scaled) so it's a strict superset of both branches below -- never culls
		// a shadow that the inner, per-branch cull would have kept. Zero visual change;
		// pure redundant-work elimination.
		{
			const Real maxExtent = MAX_SHADOW_LENGTH_EXTRA_AIRBORNE_SCALE_FACTOR * m_robjExtent;
			if (WWMath::Fabs(pos.X - bcX) > (beX + maxExtent) ||
				WWMath::Fabs(pos.Y - bcY) > (beY + maxExtent) ||
				WWMath::Fabs(pos.Z - bcZ) > (beZ + maxExtent))
				return;
		}
		...
		{
			//airborne path - early cull above already used the airborne-scaled extent, so no
			//inner cull needed.
		...
		for (j = 0; j < m_geometry->getMeshCount(); j++)
		{
			...
			if (mesh)
			{
				if (!mesh->Is_Not_Hidden_At_All())
					continue;
				const Matrix3D& meshTransform = mesh->Get_Transform();
				const Vector3 meshTranslation = meshTransform.Get_Translation();
				...
				updateMeshVolume(j, i, &meshTransform, mesh->Get_Bounding_Box(), m_robj->Get_Position().Z - zoffset);
					...
					{	//Updating the mesh volume didn't update the visibility, must do it here.
						...
						else
						{
							...
							{	//do a more accurate test against bounding box.
								...
								aaBox.Translate(meshTranslation);	//translate bounding box to world space.
...
void W3DVolumetricShadowManager::renderShadows(Bool forceStencilFill)
{
	...
	if (m_shadowList && TheGlobalData->m_useShadowVolumes)
	{
		...
		// TheSuperHackers @perf (GameClientMCP) 2026-04-17: sub-instrument the shadow volume
		// work so Cl_Fl_ShVol's ~1.8 ms (heavy fight) breaks into (a) per-shadow Update() +
		// dynamic volume submit, (b) static volume first stencil fill pass, (c) static volume
		// second pass (CCW). Tells us which bucket deserves the next cut.
		const double kShMs = PerfGetFreqQuadPartAsMsMultiplier();
		LARGE_INTEGER _shA, _shB;
		QueryPerformanceCounter(&_shA);
		...
		QueryPerformanceCounter(&_shB);
		if (kShMs > 0.0) PerfRecordLogicSub("Cl_Sh_Upd", (double)(_shB.QuadPart - _shA.QuadPart) * kShMs);
		_shA = _shB;
		...
		QueryPerformanceCounter(&_shB);
		if (kShMs > 0.0) PerfRecordLogicSub("Cl_Sh_StcA", (double)(_shB.QuadPart - _shA.QuadPart) * kShMs);
		_shA = _shB;
		...
		QueryPerformanceCounter(&_shB);
		if (kShMs > 0.0) PerfRecordLogicSub("Cl_Sh_StcB", (double)(_shB.QuadPart - _shA.QuadPart) * kShMs);
...
//		if (numRenderedShadows)
		{
			LARGE_INTEGER _qA, _qB; QueryPerformanceCounter(&_qA);
			renderStencilShadows();
			QueryPerformanceCounter(&_qB);
			if (kShMs > 0.0) PerfRecordLogicSub("Cl_Sh_Quad", (double)(_qB.QuadPart - _qA.QuadPart) * kShMs);
		}

...
static const UINT_PTR TIMER_BORDERLESS_REPOSITION = 1;
...
#define PBT_APMRESUMESUSPEND 0x0007
		...
		// ------------------------------------------------------------------------
		case WM_CLOSE:
			// Immediate process kill on Alt+F4 / window close. No graceful exit.
		...
		{
			// Intercept any attempt to move/resize the window in borderless fullscreen.
			// This catches DWM, D3D8, and Windows compatibility shims repositioning the window.
			if (!gDoPaint && TheGlobalData && !TheGlobalData->m_windowed)
			{
...
static Bool initializeAppWindows(HINSTANCE hInstance, Int nCmdShow, Bool runWindowed)
{
	...
	// Create at 800x600 centered for the splash screen bitmap.
	// The window will be resized to full screen later by Resize_And_Position_Window()
	// when the D3D device is initialized.

...
void BaseHeightMapRenderObjClass::updateShorelineTile(Int i, Int j, Int border, WorldHeightMap *pMap)
{
	...
	//Figure out maximum depth of water before we reach the effective opacity value.  Depths greater than this don't need
	...
	Real transparentDepth=TheWaterTransparency->m_transparentWaterDepth*TheWaterTransparency->getEffectiveWaterOpacity();
	if (transparentDepth <= 0.0f)
		return;
...
void BaseHeightMapRenderObjClass::initDestAlphaLUT()
{
	...
	if (surf)
	{
		...
		Int maxOpacity=(Int)(TheWaterTransparency->getEffectiveWaterOpacity() * 255.0f);
		Int alpha;
		...
		m_currentMinWaterOpacity = TheWaterTransparency->getEffectiveWaterOpacity();
	}
}
...
Int BaseHeightMapRenderObjClass::initHeightData(Int x, Int y, WorldHeightMap *pMap, RefRenderObjListIterator *pLightsIteratork, Bool updateExtraPassTiles)
{
	...
	if (updateExtraPassTiles)
	{
		...
		if (pMap)
		{
			...
			if (TheWaterTransparency->getEffectiveWaterOpacity() != m_currentMinWaterOpacity)
				initDestAlphaLUT();
		}
	}

...
// TheSuperHackers @perf (GameClientMCP) 2026-04-17: perf sink for Cl_Dr_UpdVws sub-breakdown.
...
// TheSuperHackers @mod Original max camera height before zoom mod. Used to preserve scripted
// camera behavior during non-interactive intros (letterbox mode) so they look as originally designed.
static const Real ORIGINAL_MAX_CAMERA_HEIGHT = 300.0f;
...
void W3DView::calcCameraAreaConstraints()
{
	...
	if (TheTerrainLogic)
	{
		...
		// Extra scroll allowance on the south (bottom) edge so units near the
		// bottom of the map can be inspected. ~150 world units ≈ 15 grid cells,
		// roughly the width of a large main building.
		constexpr Real extraSouthScrollAllowance = 150.0f;

		m_cameraAreaConstraints.lo.x = mapRegion.lo.x + offset;
		m_cameraAreaConstraints.hi.x = mapRegion.hi.x - offset;
		m_cameraAreaConstraints.lo.y = mapRegion.lo.y + offset - extraSouthScrollAllowance;
...
void W3DView::setCameraTransform()
{
	...
	// TheSuperHackers @mod Scale far clip plane proportionally with max camera height.
	// During non-interactive intros (letterbox mode), use the original far clip value
	// so that scripted camera sequences render with proper depth buffer precision.
	Real farZ;
	if (TheDisplay && TheDisplay->isLetterBoxed())
	{
		farZ = 1200.0f;
	}
	else
	{
		farZ = 1200.0f * (TheGlobalData->m_maxCameraHeight / ORIGINAL_MAX_CAMERA_HEIGHT);
	}
...
void W3DView::update()
{
	...
	// TheSuperHackers @perf (GameClientMCP) 2026-04-17: sub-instrument W3DView::update()
	// to isolate the 17-25 ms Cl_Dr_UpdVws spikes. Cheap QPC-pair around the two
	// big candidates: terrain heightmap full-update, and the rest of update().
	const double _uvMs = PerfGetFreqQuadPartAsMsMultiplier();
	LARGE_INTEGER _uvA = {}, _uvB = {};
	if (_uvMs > 0.0)
		QueryPerformanceCounter(&_uvA);
	...
	if (_uvMs > 0.0)
	{
		QueryPerformanceCounter(&_uvB);
		PerfRecordLogicSub("Cl_Vw_Terrain", (double)(_uvB.QuadPart - _uvA.QuadPart) * _uvMs);
		_uvA = _uvB;
	}
	...
	LARGE_INTEGER _uvC, _uvD;
	if (_uvMs > 0.0)
		QueryPerformanceCounter(&_uvC);
	TheGameClient->iterateDrawablesInRegion(&axisAlignedRegion, drawDrawable, nullptr);
	if (_uvMs > 0.0)
	{
		QueryPerformanceCounter(&_uvD);
		PerfRecordLogicSub("Cl_Vw_IterDraw", (double)(_uvD.QuadPart - _uvC.QuadPart) * _uvMs);
	}
}
...
void W3DView::draw()
{
	...
	if (doExtraRender)
	{
		...
		DX8Wrapper::Clear(false, true, Vector3(0.0f, 0.0f, 0.0f), TheWaterTransparency->getEffectiveWaterOpacity());	// Clear z but not color
...
#endif
{
	...
	if (bForceDefaultCam)
	{
		...
		TheWritableGlobalData->m_maxCameraHeight = 1000.f;
	}
	else
	{
...
{
	// TheSuperHackers @mod Start at the original default zoom level (~300) instead of max zoom-out.
	// This also preserves scripted camera sequences during non-interactive intros (letterbox mode).

	// terrain height + desired height offset == cameraOffset * actual zoom
	// find best approximation of max terrain height we can see
	Real terrainHeightMax = getHeightAroundPos(m_pos.x, m_pos.y);

	Real desiredHeightOffset = m_maxHeightAboveGround * (ORIGINAL_MAX_CAMERA_HEIGHT / TheGlobalData->m_maxCameraHeight);
	Real desiredHeight = (terrainHeightMax + desiredHeightOffset);
	...
	m_zoom = desiredZoom;
	m_heightAboveGround = desiredHeightOffset;
...
{
	// TheSuperHackers @mod Use the original max height for scripted camera zoom calculations
	// so that non-interactive intros look as originally designed with the increased max camera height.
	const Real scriptedMaxHeight = m_maxHeightAboveGround * (ORIGINAL_MAX_CAMERA_HEIGHT / TheGlobalData->m_maxCameraHeight);

	if (hasScriptedState(Scripted_Rotate))
	{
		Real terrainHeightMax = getHeightAroundPos(m_pos.x, m_pos.y);
		Real maxHeight = (terrainHeightMax + scriptedMaxHeight);
...
void W3DView::resetCamera(const Coord3D* location, Int milliseconds, Real easeIn, Real easeOut)
{
	...
	// TheSuperHackers @mod Use the original max height for scripted camera reset
	// so that non-interactive intros look as originally designed.
	const Real scriptedMaxHeight = m_maxHeightAboveGround * (ORIGINAL_MAX_CAMERA_HEIGHT / TheGlobalData->m_maxCameraHeight);

	// terrain height + desired height offset == cameraOffset * actual zoom
	// find best approximation of max terrain height we can see
	Real terrainHeightMax = getHeightAroundPos(location->x, location->y);
	Real desiredHeight = (terrainHeightMax + scriptedMaxHeight);
	Real desiredZoom = desiredHeight / m_cameraOffset.z;

...
//	float aspect = camera->Get_Aspect_Ratio();
	...
	// TheSuperHackers @bugfix Clamp calcVis arguments to valid map extent range to prevent
	// out-of-bounds access at high camera zoom levels where the visible area is very large.
	Int cvMinX = minX - WIDE_STEP/2;
	Int cvMinY = minY - WIDE_STEP/2;
	Int cvMaxX = maxX + WIDE_STEP/2;
	Int cvMaxY = maxY + WIDE_STEP/2;
	if (cvMinX < 0) cvMinX = 0;
	if (cvMinY < 0) cvMinY = 0;
	if (cvMaxX > m_map->getXExtent()) cvMaxX = m_map->getXExtent();
	if (cvMaxY > m_map->getYExtent()) cvMaxY = m_map->getYExtent();
	calcVis(frustum, m_map, cvMinX, cvMinY, cvMaxX, cvMaxY, limit);

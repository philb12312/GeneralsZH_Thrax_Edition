...
void W3DTreeBuffer::cull(const CameraClass * camera)
{
	...
	for (curTree=0; curTree<m_numTrees; curTree++) {
		...
		Bool visible = m_trees[curTree].treeType >= 0;
...
//=============================================================================
// W3DTreeBuffer::ensureTreeBuffer
//=============================================================================
Bool W3DTreeBuffer::ensureTreeBuffer(Int bufferIndex)
{
	if (bufferIndex < 0 || bufferIndex >= MAX_BUFFERS) {
		return false;
	}
	if (m_vertexTree[bufferIndex] && m_indexTree[bufferIndex]) {
		return true;
	}

	REF_PTR_RELEASE(m_vertexTree[bufferIndex]);
	REF_PTR_RELEASE(m_indexTree[bufferIndex]);

#ifdef USE_STATIC
	m_vertexTree[bufferIndex]=NEW_REF(DX8VertexBufferClass,(DX8_FVF_XYZNDUV1,MAX_TREE_VERTEX+4,DX8VertexBufferClass::USAGE_DEFAULT));
	m_indexTree[bufferIndex]=NEW_REF(DX8IndexBufferClass,(MAX_TREE_INDEX+4, DX8IndexBufferClass::USAGE_DEFAULT));
#else
	m_vertexTree[bufferIndex]=NEW_REF(DX8VertexBufferClass,(DX8_FVF_XYZNDUV1,MAX_TREE_VERTEX+4,DX8VertexBufferClass::USAGE_DYNAMIC));
	m_indexTree[bufferIndex]=NEW_REF(DX8IndexBufferClass,(MAX_TREE_INDEX+4, DX8IndexBufferClass::USAGE_DYNAMIC));
#endif

	return m_vertexTree[bufferIndex] != nullptr && m_indexTree[bufferIndex] != nullptr;
}
...
{
	if (!m_initialized || !ensureTreeBuffer(0)) {
		return;
	}
								...
								Real randomScaleAmount, const W3DTreeDrawModuleData *data)
{
	...
	m_trees[m_numTrees].visible = true;

...
void TerrainTracksRenderObjClass::init( Real width, Real length, const Char *texturename)
{
	...
	// Start with an empty bounding volume. As edges are appended via addEdgeToTrack()
	// we grow the sphere/box to cover the actual track in world space so the engine's
	// frustum culler can properly skip off-screen tracks at zoom-out.
	m_boundingSphere.Init(Vector3(0,0,0), 0.0f);
	m_boundingBox.Center.Set(0.0f, 0.0f, 0.0f);
	m_boundingBox.Extent.Set(0.0f, 0.0f, 0.0f);
	m_width=width;
	m_length=length;
	// Previously: Set_Force_Visible(TRUE); — this bypassed culling entirely, making every
	// track on the map draw every frame. With dynamic bounds (see addEdgeToTrack) the
	// engine can cull off-screen tracks properly, which is a big win at zoom-out in
	// large battles.
...
void TerrainTracksRenderObjClass::addCapEdgeToTrack(Real x, Real y)
{
	...
	m_haveCap=TRUE;
	m_haveAnchor=false;
	// Grow the world-space bounding volume for frustum culling (matches addEdgeToTrack).
	m_boundingSphere.Add_Sphere(SphereClass(topEdge.endPointPos[0], 0.0f));
	m_boundingSphere.Add_Sphere(SphereClass(topEdge.endPointPos[1], 0.0f));
}
...
void TerrainTracksRenderObjClass::addEdgeToTrack(Real x, Real y)
{
	...
	{	//no anchor yet, make this point an anchor.
		...
		// Seed the world-space bounding sphere at the anchor so the engine's frustum
		// culler can test this track. Add_Sphere() calls below will grow it as more
		// edges are added.
		m_boundingSphere.Init(m_lastAnchor, m_width);
		m_boundingBox.Center = m_lastAnchor;
		m_boundingBox.Extent.Set(m_width, m_width, m_width);
		return;
	}
	...
	// Grow the world-space bounding volume so the engine's frustum culler can test
	// this track. Include both track-width extension points so wide vehicles' tracks
	// fully fit inside the sphere. Using Add_Sphere keeps this O(1) per edge.
	m_boundingSphere.Add_Sphere(SphereClass(topEdge.endPointPos[0], 0.0f));
	m_boundingSphere.Add_Sphere(SphereClass(topEdge.endPointPos[1], 0.0f));
}

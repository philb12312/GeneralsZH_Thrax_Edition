...
static Int ApplyStandingWaterOpacity(Int diffuse)
{
	if (TheWaterTransparency == nullptr)
		return diffuse;

	Int alpha = (diffuse >> 24) & 0xff;
	const Int maxAlpha = TheWaterTransparency->getEffectiveWaterOpacityByte();

	if (alpha > maxAlpha)
		alpha = maxAlpha;

	return (diffuse & 0x00ffffff) | (alpha << 24);
}
...
HRESULT WaterRenderObjClass::generateVertexBuffer( Int sizeX, Int sizeY, Int vertexSize, Bool doStatic)
{
	...
	for (z=0; z<sizeY; z++)
	{
		for (x=0; x<sizeX; x++)
		{
			...
			pVertices->c=ApplyStandingWaterOpacity(setting->transparentWaterDiffuse);	//vertex alpha/color
			pVertices++;
		}
	}
...
//			*pData=WATER_OFFSET+WATER_AMP*(sin((float)i*WATER_FREQ*0.4+PhasePerFrameX*0.5)+sin((float)i*WATER_FREQ*0.6+PhasePerFrameX*0.2)+sin((float)j*WATER_FREQ+PhasePerFrameX)+sin((float)j*WATER_FREQ*0.7+PhasePerFrameX*0.3));
	...
	Int diffuse;
	diffuse = ApplyStandingWaterOpacity(setting->waterDiffuse);
	Int alpha = (diffuse & 0xff000000)>>24;
...
void WaterRenderObjClass::drawRiverWater(PolygonTrigger *pTrig)
{
	...
	diffuse = ApplyStandingWaterOpacity(diffuse);
...
//#ifdef WAVY_WATER // the NEW WATER a'la LORENZEN
	if ( TheGlobalData->m_featherWater )
	{
		...
		//Keep diffuse from lighting calculations but substitute custom alpha
		Int customDiffuse = ApplyStandingWaterOpacity((diffuse & 0x00ffffff) | (Alpha<< 24));//(0x80 << 16)|(0x90 << 8)|0xa0;

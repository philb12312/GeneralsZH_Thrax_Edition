...
namespace
{
SphereClass MakePropBounds(const SphereClass &sourceBounds, Real scale, const Coord3D &location)
{
	SphereClass bounds = sourceBounds;
	bounds.Center *= scale;
	bounds.Radius *= scale;
	bounds.Center += Vector3(location.x, location.y, location.z);
	return bounds;
}
}
...
void W3DPropBuffer::cull(CameraClass * camera)
{
	...
	for (curProp=0; curProp<m_numProps; curProp++) {
		m_props[curProp].visible = m_props[curProp].m_robj != nullptr;
	}
}
...
void W3DPropBuffer::addProp(Int id, Coord3D location, Real angle,Real scale, const AsciiString &modelName)
{
	...
	// Scale and translate the bounding sphere to match the actual render transform.
	m_props[m_numProps].bounds = MakePropBounds(m_propTypes[propType].m_bounds, scale, location);
...
Bool W3DPropBuffer::updatePropPosition(Int id, const Coord3D &location, Real angle, Real scale)
{
	...
	for (i=0; i<m_numProps; i++) {
		if (m_props[i].id == id) {
			...
			// Scale and translate the bounding sphere to match the actual render transform.
			m_props[i].bounds = MakePropBounds(m_propTypes[m_props[i].propType].m_bounds, scale, location);
			m_props[i].visible = true;

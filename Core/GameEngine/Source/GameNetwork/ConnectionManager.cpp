...
{
	if (IsMoneyTransferChatCommand(msg->getText()))
	{
		return;
	}
...
//	DEBUG_LOG(("ConnectionManager::processChat - got message from player %d (mask %8.8X), message is %ls", playerID, msg->getPlayerMask(), unitext.str()));
	...
	UnsignedInt playerLookupID = msg->getPlayerID();
	if (TheGameInfo)
		playerLookupID = TheGameInfo->getSharedColorOwnerSlot(playerLookupID);

	AsciiString playerName;
	playerName.format("player%d", playerLookupID);

...
#include "GameNetwork/MoneyTransfer.h"
...
namespace
{
	int g_frameGroupingCap = GENERALS_ONLINE_DEFAULT_FRAME_GROUPING_CAP;
}

int GetFrameGroupingCap()
{
	return g_frameGroupingCap;
}

void SetFrameGroupingCap(int frameGroupingCap)
{
	g_frameGroupingCap = frameGroupingCap > 0 ? frameGroupingCap : 1;
}
#endif

// PRIVATE DATA ///////////////////////////////////////////////////////////////

static Player *FindPlayerByNetworkSlot(UnsignedInt playerID, Bool useSharedColorOwner = TRUE)
{
	if (ThePlayerList == nullptr || playerID >= MAX_SLOTS)
	{
		return nullptr;
	}

	if (useSharedColorOwner && TheGameInfo)
		playerID = TheGameInfo->getSharedColorOwnerSlot(playerID);
...
enum MoneyTransferCommandType
{
	MONEY_TRANSFER_COMMAND_NONE,
	MONEY_TRANSFER_COMMAND_SEND,
	MONEY_TRANSFER_COMMAND_SENDME
};

struct MoneyTransferChatCommand
{
	MoneyTransferCommandType type;
...
static AsciiString CanonicalizeColorName(AsciiString colorName)
{
	colorName.toLower();

	AsciiString canonical;
	for (Int i = 0; i < colorName.getLength(); ++i)
	{
		char c = colorName.getCharAt(i);
	...
	if (canonical == "yellow" || canonical == "yelllow" || canonical == "yelow")
	{
		canonical = "gold";
	}
...
static Bool ColorNameMatches(AsciiString candidateName, AsciiString requestedName)
{
	AsciiString candidate = CanonicalizeColorName(candidateName);
	AsciiString requested = CanonicalizeColorName(requestedName);
	...
	if (candidate == requested || candidate.endsWith(requested.str()))
	{
		return TRUE;
	}
...
static Bool ParsePositiveAmount(const UnicodeString& amountText, UnsignedInt *amount)
{
	if (amount == nullptr || amountText.isEmpty())
	{
		return FALSE;
	}

	const UnsignedInt maxAmount = (UnsignedInt)-1;
	UnsignedInt parsedAmount = 0;
	for (Int i = 0; i < amountText.getLength(); ++i)
	{
		WideChar c = amountText.getCharAt(i);
		if (c < L'0' || c > L'9')
		{
			return FALSE;
		}

		UnsignedInt digit = c - L'0';
		if (parsedAmount > (maxAmount - digit) / 10)
		{
			return FALSE;
		}

		parsedAmount = parsedAmount * 10 + digit;
	}
...
static Bool ParseMoneyTransferChatCommand(const UnicodeString& text, MoneyTransferChatCommand *command)
{
	if (command == nullptr)
	{
		return FALSE;
	...
	command->type = MONEY_TRANSFER_COMMAND_NONE;
	command->colorName.clear();
	command->amount = 0;
	command->validSyntax = FALSE;

	UnicodeString remainingText = text;
	remainingText.trim();

	UnicodeString commandToken;
	if (!remainingText.nextToken(&commandToken))
	{
		return FALSE;
	}

	AsciiString commandName;
	commandName.translate(commandToken);
	commandName.toLower();

	if (commandName == "send")
	{
		command->type = MONEY_TRANSFER_COMMAND_SEND;

		UnicodeString colorToken;
		UnicodeString amountToken;
		UnicodeString extraToken;
		if (!remainingText.nextToken(&colorToken) || !remainingText.nextToken(&amountToken) || remainingText.nextToken(&extraToken))
		{
			return TRUE;
		}

		command->colorName.translate(colorToken);
		command->colorName = CanonicalizeColorName(command->colorName);
		command->validSyntax = !command->colorName.isEmpty() && ParsePositiveAmount(amountToken, &command->amount);
		return TRUE;
	}

	if (commandName == "sendme")
	{
		command->type = MONEY_TRANSFER_COMMAND_SENDME;

		UnicodeString amountToken;
		UnicodeString extraToken;
		if (!remainingText.nextToken(&amountToken) || remainingText.nextToken(&extraToken))
		{
			return TRUE;
		}

		command->validSyntax = ParsePositiveAmount(amountToken, &command->amount);
		return TRUE;
	}
...
Bool IsMoneyTransferChatCommand(const UnicodeString& text)
{
	MoneyTransferChatCommand command;
	return ParseMoneyTransferChatCommand(text, &command);
}

static void ShowMoneyTransferMessage(Player *recipient, const UnicodeString& message)
{
	if (recipient == nullptr || ThePlayerList == nullptr || TheInGameUI == nullptr || ThePlayerList->getLocalPlayer() != recipient)
	{
		return;
	}
...
#if defined(GENERALS_ONLINE)
	TheInGameUI->messageColor(true, &red, UnicodeString(L"%ls"), message.str());
#else
	TheInGameUI->messageColor(&red, L"%ls", message.str());
#endif
}

static void ShowMoneyTransferUsage(Player *sender)
{
	ShowMoneyTransferMessage(sender, UnicodeString(L"Usage: send [color] [amount]  or  sendme [amount]"));
}

static Bool IsBotChatVisibleToLocalPlayer(Player *bot)
{
	Player *localPlayer = ThePlayerList ? ThePlayerList->getLocalPlayer() : nullptr;
	if (localPlayer == nullptr || localPlayer == bot)
	{
		return TRUE;
	}

	const Bool botConsidersLocalAlly = (bot->getRelationship(localPlayer->getDefaultTeam()) == ALLIES);
	const Bool localConsidersBotAlly = (localPlayer->getRelationship(bot->getDefaultTeam()) == ALLIES);
	return botConsidersLocalAlly || localConsidersBotAlly;
}

static void ShowBotMoneyTransferChat(Player *bot, const UnicodeString& message)
{
	if (bot == nullptr || TheInGameUI == nullptr || message.isEmpty() || !IsBotChatVisibleToLocalPlayer(bot))
	{
		return;
	}

	UnicodeString finalLine;
	finalLine.format(L"%ls: %ls", bot->getPlayerDisplayName().str(), message.str());

	RGBColor rgb;
	rgb.setFromInt(bot->getPlayerColor() & 0x00FFFFFF);
...
static Bool AreMutualAllies(Player *first, Player *second)
{
	return first != nullptr && second != nullptr && first != second &&
		first->getRelationship(second->getDefaultTeam()) == ALLIES &&
		second->getRelationship(first->getDefaultTeam()) == ALLIES;
}

static Int GetPlayerMultiplayerColorIndex(const Player *player)
{
	if (player == nullptr || TheMultiplayerSettings == nullptr)
	{
		return -1;
	}
	...
	{
		MultiplayerColorDefinition *definition = TheMultiplayerSettings->getColor(i);
		if (definition != nullptr && definition->getColor() == player->getPlayerColor())
		{
			return i;
		}
	}
...
static Bool ColorIndexMatchesName(Int colorIndex, AsciiString requestedName)
{
	static const char* const fallbackColorNames[] = {
		"white",
		"red",
		"blue",
		"green",
	...
	if (colorIndex >= 0 && colorIndex < ARRAY_SIZE(fallbackColorNames) && ColorNameMatches(fallbackColorNames[colorIndex], requestedName))
	{
		return TRUE;
	}
	...
	{
		MultiplayerColorDefinition *definition = TheMultiplayerSettings->getColor(colorIndex);
		if (definition != nullptr && ColorNameMatches(definition->getTooltipName(), requestedName))
		{
			return TRUE;
		}
	}
...
static Bool PlayerMatchesColorName(const Player *player, AsciiString requestedName)
{
	Int colorIndex = GetPlayerMultiplayerColorIndex(player);
	return colorIndex >= 0 && ColorIndexMatchesName(colorIndex, requestedName);
}

static Player *FindAlliedPlayerByColor(Player *sender, AsciiString colorName)
{
	if (sender == nullptr || ThePlayerList == nullptr)
	{
		return nullptr;
	}
	...
	for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i)
	{
		...
		if (candidate != nullptr && candidate->isPlayerActive() && AreMutualAllies(sender, candidate) && PlayerMatchesColorName(candidate, colorName))
		{
			return candidate;
		}
	}
...
static Bool CanDepositMoney(Player *player, UnsignedInt amount)
{
	if (player == nullptr)
	{
		return FALSE;
	...
	const UnsignedInt maxAmount = (UnsignedInt)-1;
	return player->getMoney()->countMoney() <= maxAmount - amount;
}

static Bool ExecuteMoneyTransfer(Player *sender, Player *recipient, UnsignedInt amount, Player *failureRecipient, Bool suppressRecipientMessage = FALSE)
{
	...
	UnsignedInt senderBalance = sender->getMoney()->countMoney();
	if (senderBalance < amount)
	{
		UnicodeString message;
		message.format(L"Transfer failed: only $%u available.", senderBalance);
		ShowMoneyTransferMessage(failureRecipient, message);
		return FALSE;
	}

	if (!CanDepositMoney(recipient, amount))
	{
		ShowMoneyTransferMessage(failureRecipient, UnicodeString(L"Transfer failed: receiver can't hold that much."));
		return FALSE;
	}

	sender->getMoney()->withdraw(amount, FALSE);
	recipient->getMoney()->deposit(amount, FALSE, FALSE);

	UnicodeString message;
	message.format(L"Sent $%u to %ls.", amount, recipient->getPlayerDisplayName().str());
	ShowMoneyTransferMessage(sender, message);

	if (!suppressRecipientMessage)
	{
		message.format(L"Received $%u from %ls.", amount, sender->getPlayerDisplayName().str());
		ShowMoneyTransferMessage(recipient, message);
	}

	return TRUE;
}

static Bool IsActiveMutualAlliedBot(Player *requester, Player *candidate)
{
	return candidate != nullptr && candidate->isPlayerActive() && candidate->getPlayerType() == PLAYER_COMPUTER &&
		AreMutualAllies(requester, candidate);
}

static Player *FindFirstAlliedBot(Player *requester)
{
	if (requester == nullptr || ThePlayerList == nullptr)
	{
		return nullptr;
	}
	...
	{
		Player *candidate = ThePlayerList->getNthPlayer(i);
		if (IsActiveMutualAlliedBot(requester, candidate))
		{
			return candidate;
		}
	}
...
static Player *FindFirstAlliedBotWithMoney(Player *requester, UnsignedInt amount)
{
	if (requester == nullptr || ThePlayerList == nullptr)
	{
		return nullptr;
	...
	{
		Player *candidate = ThePlayerList->getNthPlayer(i);
		if (IsActiveMutualAlliedBot(requester, candidate) && candidate->getMoney()->countMoney() >= amount)
		{
			return candidate;
		}
	}
...
Bool ExecuteMoneyTransferChatCommand(Player *sender, const UnicodeString& text)
{
	MoneyTransferChatCommand command;
	if (!ParseMoneyTransferChatCommand(text, &command))
	{
		return FALSE;
	}

	if (!command.validSyntax)
	{
		ShowMoneyTransferUsage(sender);
		return TRUE;
	}
	...
	if (!sender->isPlayerActive())
	{
		ShowMoneyTransferMessage(sender, UnicodeString(L"Transfer failed: defeated players can't send money."));
		return TRUE;
	}

	if (command.type == MONEY_TRANSFER_COMMAND_SEND)
	{
		Player *recipient = FindAlliedPlayerByColor(sender, command.colorName);
		if (recipient == nullptr)
		{
			UnicodeString message;
			message.format(L"Transfer failed: no ally uses color '%hs'.", command.colorName.str());
			ShowMoneyTransferMessage(sender, message);
			return TRUE;
		}

		ExecuteMoneyTransfer(sender, recipient, command.amount, sender);
		return TRUE;
	}

	if (command.type == MONEY_TRANSFER_COMMAND_SENDME)
	{
		if (!CanDepositMoney(sender, command.amount))
		{
			ShowMoneyTransferMessage(sender, UnicodeString(L"Request failed: you can't hold that much."));
			return TRUE;
		}

		Player *bot = FindFirstAlliedBotWithMoney(sender, command.amount);
		if (bot == nullptr)
		{
			Player *answerBot = FindFirstAlliedBot(sender);
			if (answerBot != nullptr)
			{
				UnicodeString message;
				message.format(L"I only have $%u.", answerBot->getMoney()->countMoney());
				ShowBotMoneyTransferChat(answerBot, message);
			}
			else
			{
				UnicodeString message;
				message.format(L"Request failed: no allied bot has $%u.", command.amount);
				ShowMoneyTransferMessage(sender, message);
			}
			return TRUE;
		}

		if (ExecuteMoneyTransfer(bot, sender, command.amount, sender, TRUE))
		{
			UnicodeString message;
			message.format(L"Sent $%u to %ls.", command.amount, sender->getPlayerDisplayName().str());
			ShowBotMoneyTransferChat(bot, message);
		}
		return TRUE;
	}
...
static void DispatchSynchronizedChatToAI(const NetChatCommandMsg *chatMsg)
{
	if (chatMsg == nullptr || ThePlayerList == nullptr)
	{
		return;
	}

	const Player *sender = FindPlayerByNetworkSlot(chatMsg->getPlayerID());
	if (sender == nullptr || !sender->isPlayerActive())
	{
		return;
	}
	...
	for (Int i = 0; i < ThePlayerList->getPlayerCount(); ++i)
	{
		...
		if (player != nullptr)
		{
			player->onChatMessage(sender, chatMsg->getText(), chatMsg->getPlayerMask());
		}
	}
}
...
void Network::processFrameSynchronizedNetCommand(NetCommandRef* msg) {
	...
	else if (cmdMsg->getNetCommandType() == NETCOMMANDTYPE_CHAT) {
		NetChatCommandMsg *chatMsg = (NetChatCommandMsg*)cmdMsg;
		Player *sender = FindPlayerByNetworkSlot(chatMsg->getPlayerID(), FALSE);
		if (sender == nullptr)
		{
			sender = FindPlayerByNetworkSlot(chatMsg->getPlayerID());
		}
		if (!ExecuteMoneyTransferChatCommand(sender, chatMsg->getText()))
		{
			DispatchSynchronizedChatToAI(chatMsg);
		}
	}
}
...
#if defined(GENERALS_ONLINE)
	if (frameGrouping > GetFrameGroupingCap()) {
		frameGrouping = GetFrameGroupingCap(); // Max of a half a second.
	}
#else
...
void Network::processDestroyPlayerCommand(NetDestroyPlayerCommandMsg* msg)
{
	...
	if (TheGameInfo)
	{
		if (TheGameInfo->isSharedColorSecondarySlot(playerIndex))
		{
			DEBUG_LOG(("Ignoring DestroyPlayer for shared secondary slot %d from %d for frame %d on frame %d",
				msg->getPlayerIndex(), msg->getPlayerID(), msg->getExecutionFrame(), TheGameLogic->getFrame()));
			return;
		}

		playerIndex = TheGameInfo->getSharedColorOwnerSlot(playerIndex);
	}

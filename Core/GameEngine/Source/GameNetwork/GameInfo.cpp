...
static Bool isSharedColorCandidateSlot(const GameSlot *slot)
{
	...
	if (TheMultiplayerSettings && color >= TheMultiplayerSettings->getNumColors())
		return FALSE;

	return TRUE;
}

static Int findTeamForSharedColorGroup(const GameInfo *game, Int color)
{
	Bool usedTeams[MAX_SLOTS / 2];
	...
	for (Int i = 0; i < MAX_SLOTS; ++i)
	{
		...
		if (!isSharedColorCandidateSlot(slot) || slot->getColor() == color)
			continue;
	...
	return (MAX_SLOTS / 2) > 0 ? color % (MAX_SLOTS / 2) : -1;
}

Int GameInfo::getSharedColorOwnerSlot(Int slotNum) const
{
	if (slotNum < 0 || slotNum >= MAX_SLOTS)
		return slotNum;

	const GameSlot *slot = getConstSlot(slotNum);
	if (!isSharedColorCandidateSlot(slot))
		return slotNum;

	Int color = slot->getColor();
	Int firstSlot = -1;
	Int firstHumanSlot = -1;
	for (Int i = 0; i < MAX_SLOTS; ++i)
	{
		const GameSlot *checkSlot = getConstSlot(i);
		if (!isSharedColorCandidateSlot(checkSlot) || checkSlot->getColor() != color)
			continue;

		if (firstSlot < 0)
			firstSlot = i;

		if (checkSlot->isHuman())
		{
			firstHumanSlot = i;
			break;
		}
	...
	if (firstHumanSlot >= 0)
		return firstHumanSlot;

	return firstSlot >= 0 ? firstSlot : slotNum;
}

Bool GameInfo::isSharedColorSecondarySlot(Int slotNum) const
{
	return getSharedColorOwnerSlot(slotNum) != slotNum;
}

void GameInfo::normalizeSharedColorSlots()
{
	for (Int ownerSlot = 0; ownerSlot < MAX_SLOTS; ++ownerSlot)
	{
		GameSlot *owner = getSlot(ownerSlot);
		if (!isSharedColorCandidateSlot(owner) || getSharedColorOwnerSlot(ownerSlot) != ownerSlot)
			continue;

		Int color = owner->getColor();
		Bool hasSharedSlot = FALSE;
		Int sharedTeam = owner->getTeamNumber();
		Int sharedStart = owner->getStartPos();

		for (Int i = 0; i < MAX_SLOTS; ++i)
		{
			GameSlot *slot = getSlot(i);
			if (!isSharedColorCandidateSlot(slot) || slot->getColor() != color || i == ownerSlot)
				continue;

			hasSharedSlot = TRUE;
			if ((sharedTeam < 0 || sharedTeam >= MAX_SLOTS / 2) && slot->getTeamNumber() >= 0 && slot->getTeamNumber() < MAX_SLOTS / 2)
				sharedTeam = slot->getTeamNumber();
			if (sharedStart < 0 && slot->getStartPos() >= 0)
				sharedStart = slot->getStartPos();
		}

		if (!hasSharedSlot)
			continue;

		if (sharedTeam < 0 || sharedTeam >= MAX_SLOTS / 2)
			sharedTeam = findTeamForSharedColorGroup(this, color);
		...
		for (Int i = 0; i < MAX_SLOTS; ++i)
		{
			...
			if (sharedTeam >= 0)
				slot->setTeamNumber(sharedTeam);
			if (sharedStart >= 0)
				slot->setStartPos(sharedStart);
		}
	}
}

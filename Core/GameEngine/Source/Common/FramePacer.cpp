...
void FramePacer::setFramesPerSecondLimit( Int fps )
{
	...
	fps = std::max<int>(fps, LOGICFRAMES_PER_SECOND); // TheSuperHackers @mod Use logic tick rate as floor, not GENERALS_ONLINE_HIGH_FPS_LIMIT
#endif

	m_maxFPS = fps;

	// TheSuperHackers @mod Auto-enable logic time scale decoupling when render FPS exceeds logic FPS.
	// This prevents game logic from running faster than intended at high frame rates.
	if (m_maxFPS > LOGICFRAMES_PER_SECOND)
	{

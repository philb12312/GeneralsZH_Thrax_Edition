...
enum AsyncPathfindMode CPP_11(: Int)
{
	ASYNC_PATH_MODE_MOVE = 0,
	ASYNC_PATH_MODE_ATTACK,
	ASYNC_PATH_MODE_SAFE
};

struct AsyncPathfindJob
{
	ObjectID objectID;
	UnsignedInt requestSerial;
	UnsignedInt submitFrame;
	AsyncPathfindMode mode;
	Coord3D from;
	Coord3D to;
	...
	Bool centerInCell;
	Bool isCrusher;
	Bool canPathThroughUnits;
	Bool isFinalGoal;
	Bool ultraAccurate;
	Bool preferClosest;
	Bool requireAttackLine;
	Bool provisionalAllowed;
	ObjectID ignoreObstacleID;
};
...
struct AsyncPathfindResult
{
	...
	Bool ultraAccurate;
	Int cellsExamined;
	std::vector<ICoord2D> pathCells;
	std::vector<PathfindLayerEnum> pathLayers;
	std::vector<Bool> pathCanOptimize;
};

class AsyncPathfindWorker;
...
public:
	Pathfinder();
	~Pathfinder() ;
	PathfindServicesInterface *getPathfindServices() { return this; }
	...
	void processPathfindQueue(); ///< Process some or all of the queued pathfinds synchronously.
	void serviceAsyncPathfindQueue(); ///< Apply finished worker paths and submit eligible queued paths.
	void stopAsyncPathfindWorker(); ///< Stop worker-thread pathfinding before reset/shutdown.
...
private:
	void ensureAsyncPathfindWorker();
	Bool isAsyncPathfindAllowed() const;
	void collectAsyncPathfindResults();
	void updateLogicalExtentForPathfindQueue();
	Bool submitAsyncPathfindJobs();
	...
	AsyncPathfindWorker *m_asyncPathfindWorker;

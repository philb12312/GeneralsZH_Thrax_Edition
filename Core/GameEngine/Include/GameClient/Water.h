...
class WaterTransparencySetting : public Overridable
{
	...
	public:
		...
		// Caps the drawn water surface opacity, leaving roughly 30% see-through by default.
		Real m_standingWaterOpacity;
	...
	public:
		...
		Real getEffectiveWaterOpacity() const
		{
			Real opacity = m_minWaterOpacity;

			if (m_standingWaterOpacity < opacity)
				opacity = m_standingWaterOpacity;
		...
		UnsignedByte getEffectiveWaterOpacityByte() const
		{
			return (UnsignedByte)(getEffectiveWaterOpacity() * 255.0f + 0.5f);
		}
};

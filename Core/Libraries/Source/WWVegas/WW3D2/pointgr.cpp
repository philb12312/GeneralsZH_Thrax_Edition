	...
	while (current<vnum)
	{
		...
		// Copy in the data to the VB
		{
			...
			const FVFInfoClass& fvfinfo=PointVerts.FVF_Info();
				const int loc_offset=fvfinfo.Get_Location_Offset();
				const int diffuse_offset=fvfinfo.Get_Diffuse_Offset();
				const int tex_offset=fvfinfo.Get_Tex_Offset(0);
				const int fvf_size=fvfinfo.Get_FVF_Size();
				const unsigned default_color=DX8Wrapper::Convert_Color_Clamp(Vector4(DefaultPointColor[0],DefaultPointColor[1],DefaultPointColor[2],DefaultPointAlpha));
				const Vector3 *src_loc=&VertexLoc[current];
				const Vector4 *src_diffuse=&VertexDiffuse[current];
				const Vector2 *src_uv=&VertexUV[current];
			...
			for (i = current; i < current + delta; i++)
			{
				...
				// Copy Locations
					*(Vector3*)(vb+loc_offset)=*src_loc++;
				if (current_diffuse) {
						unsigned color=DX8Wrapper::Convert_Color_Clamp(*src_diffuse++);
						*(unsigned int*)(vb+diffuse_offset)=color;
				}
				else
						*(unsigned int*)(vb+diffuse_offset)=default_color;
					*(Vector2*)(vb+tex_offset)=*src_uv++;
					vb+=fvf_size;
			}
		}
